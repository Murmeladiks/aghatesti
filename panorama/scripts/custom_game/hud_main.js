...
function OnIntroduceEncounter(data) {
	...
	if (data["ascension_abilities"] != null) {
		for (var key of Object.keys(data["ascension_abilities"])) {
			...
			hAbilityPanel.SetPanelEvent("onmouseover", () => {
				$.DispatchEvent("DOTAShowTextTooltip", hAbilityPanel, $.Localize("#" + `${abilityName}_Description`));
			});

			hAbilityPanel.SetPanelEvent("onmouseout", () => {
				$.DispatchEvent("DOTAHideTextTooltip", hAbilityPanel);
			});
...
function OnBossUpdated() {
	...
	if (CurrentBoss) {
		const container = $("#BossHP");
		container.RemoveAndDeleteChildren();
		container.bars = [];

		let label = $.CreatePanel("Label", container, "BossLabel");
		label.text = $.Localize("#" + CurrentBoss.unit_name);

		const bosses = Object.values(CurrentBoss.ent_index);
		for (const unit of bosses) {
			const bar = $.CreatePanel("Panel", container, unit);
			bar.BLoadLayoutSnippet("BossHealthBar");
			bar.SwitchClass("class", "Boss_" + CurrentBoss.unit_name);
			bar.unit = unit;
...
function BossThink() {
	...
	for (const bar of container.bars) {
		const is_alive = Entities.IsAlive(bar.unit);
		bar.FindChildTraverse("BossProgressBar").value = is_alive
			? Entities.GetHealth(bar.unit) / Entities.GetMaxHealth(bar.unit)
			: 0;

		if (is_alive) active++;
	}

	$("#BossHealthBarContainer").SetHasClass("Visible", active > 0 && g_bInBossCamera == false);

	if (active > 0) {
		$.Schedule(0.1, BossThink);
	}
}
...
function ShowWikiHome(kv) {
	let list = Object.keys(kv);
	list.splice(list.indexOf("splitscreenplayer"), 1);
	list.sort()

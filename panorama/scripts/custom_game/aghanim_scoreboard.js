...
function UpdateStats() {
	for (let nPlayerID = 0; nPlayerID < 4; ++nPlayerID) {
		let playerData = CustomNetTables.GetTableValue("aghanim_scores", nPlayerID.toString());
		if (playerData == null) {
...
function UpdateBlessings() {
	...
	Object.keys(blessingsData).forEach(function (key) {
		...
		Object.keys(blessingsData[key]).forEach(function (subkey) {
			...
			blessing.SetDialogVariable(
				"blessing_description",
				$.Localize("#DOTA_TI10_EventGame_" + szActionName + "_Desc"),
			);
		});
	});
}
...
function InitializeScoreboard() {
	...
	for (i; i < 4; i++) {
		...
		if (playerRow !== null) {
			...
			playerHeroNameLabel.text = $.Localize("#" + Players.GetPlayerSelectedHero(playerID));

			playerRow.SetHasClass("NoSelectedHero", playerHeroNameLabel.text === "invalid index");
		...
		var blessingsRow =
			$("#BlessingPlayers").FindChildInLayoutFile(playerID.toString()) ||
			$.CreatePanel("Panel", $("#BlessingPlayers"), playerID.toString());
...
function UpdateRoomRewards() {
	...
	for (var i = 0; i < hAllRoomData.length; ++i) {
		...
		if (Number(roomData["completed"]) != 1 && Number(roomData["current_room"]) != 1) continue;

		if (roomData["room_type"] == 5 || roomData["room_type"] == 6) continue;

		if (roomData["reward"] == null) continue;
	...
	for (j; j < 4; j++) {
		...
		for (i; i <= nCurrentDepth; i++) {
			...
			if (RewardAbilityImage) {
				...
				RewardAbilityImage.SetDialogVariable(
					"reward_description",
					GetDescription(RewardChoice, RewardAbilityImage),
				);
			}
			...
			if (
				RewardChoice["reward_type"] == "REWARD_TYPE_MINOR_ABILITY_UPGRADE" ||
				...
				claimedRewardPanel.SetDialogVariable(
					"value",
					Math.floor(flValue) == flValue ? Math.floor(flValue) : flValue.toFixed(1),
				);*/
			} else {
				//custom ping code
				let shard_text = $.Localize(
					"#DOTA_Tooltip_ability_" + RewardAbilityImage.abilityname,
					claimedRewardPanel,
				);
			...
			claimedRewardPanel.SetDialogVariable(
				"tier",
				$.Localize("#DOTA_HUD_" + RewardChoice["reward_tier"] + "_Desc"),
			);

			claimedRewardPanel.SetDialogVariable("reward_name", GetName(RewardChoice, claimedRewardPanel));
			claimedRewardPanel.SetDialogVariable(
				"reward_description",
				GetDescription(RewardChoice, claimedRewardPanel),
			);
...
function TipPlayerEvent(target_id) {

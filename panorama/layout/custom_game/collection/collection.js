...
let WHEEL_CONTAINER = $("#WheelLinesList");
let active_button_id = -1;
let bChatWheelAccess = false;
	...
	$("#ItemsList")
		...
		.forEach((panel, index) => {
			...
			for (const item of items) {
				let state = 0;
				if (item.category != "Treasures") {
					state = item.BHasClass("equipped") ? EQUIPPED : EQUIP;
					AddItemToAvailebleList(item.itemName, state, 1);
...
function UpdatePlayerInfo() {
	...
	playerBooster.text = $.Localize("#player_" + PLAYER_BOOST_STATE[2]);
	Object.values(PLAYER_BOOST_STATE).forEach((className) => {
		playerBooster.RemoveClass(className);
	});
	playerBooster.AddClass(PLAYER_BOOST_STATE[2]);
	const playerProgressText = $("#DotaUBP_LevelProgress");
	playerProgressText.SetDialogVariable("curr_exp", 0);
	playerProgressText.SetDialogVariable("max_exp", 1000);
	$("#BPProgressBar").value = 0 / 1000;
	$("#DotaUBP_PlayerLevel").text = 1;
	UpdateCoins({ coins: 2000 });
	ShowBoostInfo("GoldBoost");

	$("#BuyBoost_base_booster").SetHasClass("block", true);
	$("#BuyBoost_base_booster").AddClass("block");
	$("#BuyBoost_golden_booster").AddClass("block");

	const donateTimeLeft = (new Date(0) - new Date()) / 864e5;
	...
	$("#BoostTimeLeftRoot_2").visible = donateTimeLeft > 0;
}
...
function InitCollection(_data) {
	...
	ITEMS_TYPES.forEach((itemTypeName) => {
		...
		if (itemTypeName == "ChatWheels") {
			LoadChatWheel(itemTypePanel, itemsListWrap);
		}
	});
			...
			const createItemTooltip = (panel, bShowSource) => {
				...
				panel.SetPanelEvent("onmouseover", () => {
					$.DispatchEvent(
						...
						"file://{resources}/layout/custom_game/collection/collection_item_tooltip/collection_item_tooltip.xml",
						params,
					);
...
function OpenSpecificCollection(data) {
	...
	if (data.specific_supp_name) {
		ShowBoostInfo(data.specific_supp_name);
	}
...
function LoadChatWheel(button, panel) {
	if (!bChatWheelAccess) return;
	const VoiceLinePicker = $("#VoiceLinePicker");
	button.visible = true;
	VoiceLinePicker.visible = true;
	panel.FindChildTraverse("ItemNone").visible = false;
	VoiceLinePicker.SetParent(panel);

	GameEvents.SendCustomGameEventToClient("load_chatwheel", Game.GetLocalPlayerID(), {});
	GameEvents.SendCustomGameEventToServer("ChatWheel:RequestVoiceLinePacks", {});
}

/**************************************************/

function ReloadWheel(force_owned_only, search_word) {
    FillSavedLines();
    ClearVoiceLines();
    PopulateVoiceLines(force_owned_only, search_word);
}

/**************************************************/

function UpdateVoiceLines() {
    GameEvents.SendCustomGameEventToServer("ChatWheel:UpdateVoiceLines", {active_lines: active_lines});
}

/**************************************************/

function PopulateVoiceLines(force_owned_only, search_word) {
    for (const category in CATEGORIES) {
        const lines = CATEGORIES[category];
        let category_owned = false;
        let bundle_panel = $.CreatePanel("Panel", $("#WheelLinesList"), "WheelLinesBundle");
        let category_panel = $.CreatePanel("Panel", bundle_panel, "WheelLineCategory");
		let search_word_found = false;
        
        category_panel.BLoadLayoutSnippet("LineCategory");
        category_panel.FindChildTraverse("HeaderTitle").text = $.Localize(category);
        
        for (const lineHeader of lines) {
            if (!VOICE_LINES.hasOwnProperty(lineHeader)) continue;

            if (bChatWheelAccess || unlocked_lines.includes(lineHeader)) {
                category_owned = true;
            } else if (force_owned_only) continue;
			
            let voiceLineInfo = VOICE_LINES[lineHeader];

			if (
				search_word !== undefined && 
				!(
					voiceLineInfo.message?.toLowerCase().includes(search_word.toLowerCase()) ||
					$.Localize(voiceLineInfo.translation)?.toLowerCase().includes(search_word.toLowerCase()) ||
					voiceLineInfo.source?.toLowerCase().includes(search_word.toLowerCase())
				)
			) {
				empty_search = true;
				continue;
            ...
            let phrase_panel = $.CreatePanel("Panel", bundle_panel, "WheelPhraseLine");

            phrase_panel.BLoadLayoutSnippet("PhraseLine");

            let message = $.Localize(voiceLineInfo.message);
            let translation = voiceLineInfo.translation != undefined ? $.Localize(voiceLineInfo.translation) : "";

            if (translation == message) translation = ""; 

            let info_panel = phrase_panel.GetChild(1);
            info_panel.GetChild(0).text = translation;
            info_panel.GetChild(1).text = voiceLineInfo.source ? "- " + voiceLineInfo.source : "";

            phrase_panel.FindChildTraverse("WheelPhraseText").text = $.Localize(voiceLineInfo.message);

            let sound_button = phrase_panel.FindChildTraverse("ChatWheelSoundButton");

            sound_button.SetPanelEvent("onactivate", () => {
                Game.EmitSound(voiceLineInfo.sound);
            });

            sound_button.SetPanelEvent("onmouseover", () => {
                $.DispatchEvent(
                    "DOTAShowTextTooltip",
                    sound_button,
                    "Play Sound"
            ...
            sound_button.SetPanelEvent("onmouseout", () => {
                $.DispatchEvent("DOTAHideTextTooltip");
            });

            if (bChatWheelAccess || unlocked_lines.includes(lineHeader)) {
                phrase_panel.AddClass("LineIsUnlocked");
            }

            if (force_owned_only || search_word != undefined) {
                phrase_panel.SetPanelEvent("onactivate", () => {
                    LinePicked(lineHeader, voiceLineInfo.label, voiceLineInfo.sound);
                });
            }
        }
		if (search_word != undefined && !search_word_found) bundle_panel.DeleteAsync(0);

        else if (!category_owned && (HideLockedLines || force_owned_only)) bundle_panel.DeleteAsync(0);
    }
}

/**************************************************/

function ClearVoiceLines() {
    for (let i = 0; i < $("#WheelLinesList").GetChildCount(); i++) {
        $("#WheelLinesList").GetChild(i).DeleteAsync(0);
    } 
}

/**************************************************/

function SearchVoiceLines() {
	let text = $("#LineSearchEntry").text;
	
	ClearVoiceLines();
	PopulateVoiceLines(false, text);
}

/**************************************************/

function SearchVoiceFocus() {
	$("#LineSearchPlaceholder").style.opacity = 0;
}

/**************************************************/

function SearchVoiceBlur() {
	if ($("#LineSearchEntry").text == "")
		$("#LineSearchPlaceholder").style.opacity = 1;
}

/**************************************************/

function ClearLineSearch() {
	$("#LineSearchEntry").text = "";

	$.DispatchEvent("DropInputFocus", $("#LineSearchEntry"));
	ClearVoiceLines();
    PopulateVoiceLines();
}
...
function LinePicked(line_id, line_label, line_sound) {
    if (!IsCurrentlySelectingLine()) return;
    active_lines[active_button_id][0] = line_label;
    active_lines[active_button_id][1] = line_sound;
    active_lines[active_button_id][2] = line_id;
    UnselectButton();
    FillSavedLines();
}
...
function PickLineForButton(button_id) {
	if (button_id == active_button_id) {UnselectButton(); return}
    if (IsCurrentlySelectingLine()) UnselectButton();

    let button = $("#WheelButton" + button_id)
    button.AddClass("ChoosingLine");
    $("#WheelChoicesContainer").AddClass("Active");
    $("#WheelChoicesContainer").style.backgroundColor = "gradient( linear, 0% 0%, 0% 100%, from( #452354 ), to( #1F2533 ) )";
    $("#WheelChoicesContainer").style.boxShadow = "#000000dd 0px 0px 12px 0px";

    active_button_id = button_id;
    ReloadWheel(true, $("#LineSearchEntry").text);
}

/**************************************************/

function ClearLine(button_id) {
    active_lines[button_id] = ["", ""];
    FillSavedLines();
    Game.EmitSound("General.ButtonClick");
}
...
function UnselectButton() {
    if (!IsCurrentlySelectingLine())  return;
    $("#WheelButton" + active_button_id).RemoveClass("ChoosingLine");
    $("#WheelChoicesContainer").RemoveClass("Active");
    $("#WheelChoicesContainer").style.backgroundColor = "gradient(linear, 0% 0%, 0% 100%, from(#202023), to(#202023))";
    $("#WheelChoicesContainer").style.boxShadow = "#00000033 0px 0px 4px 0px";
    active_button_id = -1;
    ReloadWheel(HideLockedLines, $("#LineSearchEntry").text);
}

/**************************************************/

function IsCurrentlySelectingLine() {
    return active_button_id > -1;
}

/**************************************************/

function FillSavedLines() {
    for (let i = 0; i < 9; i++) {
        let button = $("#WheelButton" + i);

        if (active_lines[i] != undefined) {
            button.text = $.Localize(active_lines[i][0]);
        }
...
let HideLockedLines = false;

/**************************************************/

function SwitchLockedLines(sound) {
	if (!HideLockedLines) {
		HideLockedLines = true;
		$("#HideLockedLinesTogglePillCircle").style.transform = "translateX(24px)";
		$("#HideLockedLinesTogglePillActive").style.width = "100%";
	} else {
		HideLockedLines = false;
		$("#HideLockedLinesTogglePillCircle").style.transform = "none";
		$("#HideLockedLinesTogglePillActive").style.width = "0%";
	}

    ReloadWheel(IsCurrentlySelectingLine() || HideLockedLines);
}

/**************************************************/

function VoiceLinesAcquired(event) {
    let temp = event.data;
    for (let key in temp) {
        if (temp.hasOwnProperty(key)) {
            VOICE_LINES = { ...VOICE_LINES, ...temp[key] };
        }
    }
    PopulateVoiceLines(false)
}

/**************************************************/

function CreateVoiceLineMessage(event) {
    RequestAllGuildNames()
    if (event.player_id < 0) return;

	let message_panel = NewChatLine();
    const playerInfo = Game.GetPlayerInfo(event.player_id);
    let player_name = GameUI.CustomUIConfig().player_fullnames[event.player_id];
    let text =  `<font color="${GetRealPlayerColor(event.player_id)}">${player_name}</font><font color="#faeac9">:</font> `;
    
    $.CreatePanel("Panel", message_panel, "", { class: "HeroBadge PlusHeroBadgeIconSmall", selectionpos: "auto" });

    const heroIcon = $.CreatePanel("Image", message_panel, "", { class: "HeroIcon", selectionpos: "auto" });
    heroIcon.SetImage("file://{images}/heroes/" + playerInfo.player_selected_hero + ".png");
    heroIcon.style.marginTop = "1px"

    let real_text = $.CreatePanel("Label", message_panel, "", {
		class: "ChatLine",
		html: "true",
	});

    real_text.style.width = "fit-children";
    real_text.style.verticalAlign = "top"
    real_text.text = text;

    let chatwheel_message = $.CreatePanel("DOTAChatWheelMessage", message_panel, "ChatWheel_Custom");

    $.Schedule(1/30, () => {
        chatwheel_message.AddClass("HasSource");
        chatwheel_message.AddClass("HasSound");
    
        chatwheel_message.SetAttributeString("vertical-align", "top");
        chatwheel_message.SetDialogVariable("text", event.message);
        chatwheel_message.SetDialogVariable("source", event.source);
        if (event.translation != undefined) {
			let sTranslatedText = $.Localize(event.translation);
			if (sTranslatedText != event.message) {
				chatwheel_message.AddClass("HasTranslation");
				chatwheel_message.SetDialogVariable("translation", $.Localize(event.translation));
			}
        }
    })
...
function NewChatLine() {
	let chat_panel = FindDotaHudElement("ChatLinesPanel");

	let chat_line = $.CreatePanel("Panel", chat_panel, "", {
		class: "ChatLine",
		html: "true",
		selectionpos: "auto",
		hittest: "false",
	...
	chat_line.style.flowChildren = "right";
	chat_line.style.color = "#faeac9";
    chat_line.SetAttributeString("ChatTime", Game.GetGameTime().toFixed(3));
...
function SetChatWheelAccess(event) {
	bChatWheelAccess = event.access;
}

/*****************************************************************************/

(function () {
	GameEvents.SendCustomGameEventToServer("ChatWheel:RequestPlayerAccessStatus", {}); 
	...
	GameEvents.Subscribe("ChatWheel:SetupLines", VoiceLinesAcquired);
	GameEvents.Subscribe("ChatWheel:EmitVoiceLine", CreateVoiceLineMessage); 
	GameEvents.Subscribe("ChatWheel:PlayerAccessStatus", SetChatWheelAccess);

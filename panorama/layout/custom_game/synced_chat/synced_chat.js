...
let sc_top_button = FindDotaHudElement("OpenSyncedChat");
...
SubscribeToNetTableKey("game", `player_supporter_${LOCAL_PLAYER_ID}`, function (patreon_bonuses) {
	let level = 0;
	if (patreon_bonuses && patreon_bonuses.level) {
		level = patreon_bonuses.level;
	}
...
(function () {
	...
	GameEvents.Subscribe("synced_chat:toggle", ToggleSyncedChat);
})();

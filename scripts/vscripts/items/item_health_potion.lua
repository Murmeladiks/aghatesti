...
function item_health_potion:OnSpellStart()
    if IsServer() then
        ...
        for _, Hero in pairs(Heroes) do
            if Hero ~= nil and not Hero:HasModifier("modifier_hero_select") and Hero:IsRealHero() and Hero:IsAlive() and
                Hero:GetTeamNumber() == nTeamNumber then
                ...
                local nFXIndex = ParticleManager:CreateParticle("particles/items3_fx/fish_bones_active.vpcf",
...
function item_health_potion:CanUnitPickUp(hUnit)
    if hUnit ~= nil and hUnit:IsNull() == false and hUnit:IsAlive() and
        (hUnit:IsRealHero() or (hUnit:IsOwnedByAnyPlayer() and hUnit:IsCreepHero())) then
        return true
    end

	...
	if IsServer() then
		if self:GetCaster() ~= nil and self:GetCaster():IsRealHero() then
			...
			for nPlayerID = 0,AGHANIM_PLAYERS-1 do
				...
				if hPlayerHero then
					if hPlayerHero:HasAbility("winter_wyvern_eldwurm_scholar") then
						nMultiplier = nMultiplier + hPlayerHero:FindAbilityByName("winter_wyvern_eldwurm_scholar"):GetSpecialValueFor("share_percentage") / 100 
					end
				end
			end

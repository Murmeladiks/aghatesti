...
function CMapEncounter_TrapBase:GetBlacklistedTrapModifiers()
	...
	{
		"winter_wyvern_pf_arctic_burn_active",
		"modifier_phoenix_sun_ray_pf_caster_dummy",
		"modifier_templar_assassin_pf_refraction_absorb",
		"modifier_aghsfort_viper_corrosive_skin_flying",
		"modifier_phoenix_icarus_dive_dash_dummy_pf",
	}
...
function CMapEncounter_TrapBase:Start()
	...
	for _, hHero in pairs ( hHeroes ) do
		if hHero ~= nil  and not hHero:HasModifier("modifier_hero_select") and not hHero:IsNull() and hHero:IsRealHero() then
			...
			for _,szModifierName in pairs ( vecBlacklistedModiifers ) do 
				hHero:RemoveModifierByName( szModifierName )
			end
	...
	if SPAWN_TRAP_CATAPULT and RandomInt(1,100) < 100 and PlayerResource:GetPlayerCountForTeam(DOTA_TEAM_GOODGUYS) > 2 then
		...
		if button  then
			local hUnit = CreateUnitByName( "pathfinder_trap_catapult", button:GetAbsOrigin() + RandomVector(200), true, nil, nil, DOTA_TEAM_BADGUYS )
...
function CMapEncounter_TrapBase:CheckForCompletion()
	...
	for _, hHero in pairs ( hHeroes ) do
		if hHero ~= nil and hHero:IsRealHero() and hHero:GetTeamNumber() == DOTA_TEAM_GOODGUYS and not hHero:HasModifier("modifier_hero_select")  and hHero:GetUnitName() ~= "npc_dota_hero_wisp" then
			if hHero:IsAlive() or hHero:GetRespawnsDisabled() == false and not hHero:HasOwnerAbandoned() then
	...
	if self.catapos then
		for i=1,1 do
			if IsServer() then
				...
				local dropTarget = self.catapos + RandomVector( Script_RandomFloat( 125, 175 ) )

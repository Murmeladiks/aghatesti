	...
	for _, hHero in pairs ( hHeroes ) do
		if hHero ~= nil and not hHero:HasModifier("modifier_hero_select")  and not hHero:IsNull() and hHero:IsRealHero() then
			...
			if hAbility ~= nil then
				...
				hHero:AddNewModifier( hHero, nil, "modifier_not_debuff_immune", {duration = -1})
			else
...
function CMapEncounter_Pangolier:OnComplete()
	...
	for _, hHero in pairs ( hHeroes ) do
		if hHero ~= nil  and not hHero:HasModifier("modifier_hero_select") and not hHero:IsNull() and hHero:IsRealHero()  then
			if hHero:HasModifier("modifier_obsidian_destroyer_astral_imprisonment_prison") then
				hHero:RemoveModifierByName("modifier_obsidian_destroyer_astral_imprisonment_prison")
			end
			...
			hHero:RemoveModifierByName( "modifier_not_debuff_immune" )

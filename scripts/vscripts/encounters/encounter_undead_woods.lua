...
function CMapEncounter_UndeadWoods:constructor( hRoom, szEncounterName )
	...
	local flPortalIntervalInput = DEFAULT_PORTAL_SPAWN_INTERVAL + 10
	...
	for i=1,self.nNumPortals do
		...
		self:AddPortalSpawner( CPortalSpawner( name, "dynamic_portal", 220 * hRoom:GetDepth(), flInitialPortalSpawnDelay, flInitialSummonTime, flPortalIntervalInput, flScaleInput,
			{
				{
					EntityName = "npc_dota_undead_woods_skeleton_king",
					Team = DOTA_TEAM_BADGUYS,
...
function CMapEncounter_UndeadWoods:Start()
	...
	local wk_sound = {"wk_name",
						"wk_follow"}

	EmitManySounds(wk_sound, 2.15)
end
...
--------------------------------------------------------------------------------
function CMapEncounter_UndeadWoods:OnComplete()
	CMapEncounter.OnComplete(self)
	EmitAnnouncerSound("wk_done")
end

...
function CMapEncounter_NagaSiren:Start()
	...
	local intro_sound = {"naga_name",
						"naga_follow"}

	-- EmitManySounds(intro_sound, 1.5)
end
...
function CMapEncounter_NagaSiren:CheckForCompletion()
	...
	if not self.bInitialSpawn or not self.bAllowedToFinish then
		return false
	end

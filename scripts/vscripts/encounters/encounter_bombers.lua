...
function CMapEncounter_Bombers:InitializeObjectives()
	...
	print("MAX ENEMY COUNT FOR THIS ENCOUNTER: "..self:GetMaxSpawnedUnitCount())
end
...
function CMapEncounter_Bombers:OnRequiredEnemyKilled( hAttacker, hVictim )
	...
	print(hVictim:GetUnitName() .. " has been KILLED!")
end

--------------------------------------------------------------------------------

function CMapEncounter_Bombers:MustKillForEncounterCompletion(hEnemyCreature)
	local sCreatureName = hEnemyCreature.GetUnitName and hEnemyCreature:GetUnitName() or ""

	if hEnemyCreature:HasModifier("modifier_pathfinder_revenant_status") then
		return false
	end

	if sCreatureName ~= "npc_dota_creature_bomber" and sCreatureName ~= "npc_dota_creature_gyrocopter" then
		return false
	end

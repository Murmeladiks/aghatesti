...
function CMapEncounter_StartingRoom:Start()
    ...
    if GameRules.Aghanim:HasSetAscensionLevel() == false then
        ...
        while nOption <= nMaxOption + 1 do
            ...
            if nOption < 2 then
                nSelectorFX = ParticleManager:CreateParticle("particles/agh_outpost_radiant_ambient.vpcf", PATTACH_ABSORIGIN_FOLLOW, picker)
                picker:SetMaterialGroup("1")
            else
                nSelectorFX = ParticleManager:CreateParticle("particles/agh_outpost_dire_ambient.vpcf", PATTACH_ABSORIGIN_FOLLOW, picker)
                picker:SetMaterialGroup("2")
            end

            ParticleManager:SetParticleControlEnt(nSelectorFX, 0, picker, PATTACH_POINT_FOLLOW, "attach_fx", picker:GetAbsOrigin(), true)
            local hModifier = picker:FindModifierByName("modifier_aghsfort_watch_tower")
            if hModifier then
                hModifier:AddParticle(nSelectorFX, false, false, -1, false, false)
            end
            --------------------------
...
function CMapEncounter_StartingRoom:OnComplete()
	...
	-- Show perk selection for all players
	PerkManager:StartAllowingPerkSelection()

	local vecPlayers = GameRules.Aghanim:GetConnectedPlayers()
	for _, player_id in pairs(vecPlayers) do
		PerkManager:ShowPerkSelectionForPlayer(player_id)
	end
    ...
    for nPlayerID = 0, AGHANIM_PLAYERS - 1 do
        ...
        if hHero then
            ...
            if not (hHero:GetName() == "npc_dota_hero_ogre_magi" and hHero:GetHeroFacetID() == 2) then
            ...
            if hHero:HasAbility("marci_pf_special_delivery") then
                Timers:CreateTimer(5, function()
                    GameRules.Aghanim:MarciReward(hHero:GetPlayerOwnerID())
                end)
            end
        end

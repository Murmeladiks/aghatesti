...
function CMapEncounter_Wrath:GetNextLightChangeTime()
	return Script_RandomFloat( 1.0, 7.0 )
end
...
function CMapEncounter_Wrath:CheckForCompletion()
	...
	for _, hHero in pairs ( hHeroes ) do
		if hHero ~= nil  and not hHero:HasModifier("modifier_hero_select") and hHero:IsRealHero() and hHero:GetTeamNumber() == DOTA_TEAM_GOODGUYS and ( hHero:IsAlive() or hHero:IsReincarnating() )  then

...
function CMapEncounter_PenguinsTransition:Start()
	...
	for _, hBagTarget in pairs( self.hPreplacedBagTargets ) do
		...
		for i = 1, nCoinSpawnsPerTarget do
			...
			newItem:LaunchLoot(true, fHeight, 0.75, vGoldBagPos, nil)
	 		table.insert( self.GoldBags, newItem )
			self.nTotalGoldBagsSpawned = self.nTotalGoldBagsSpawned + 1
		end
	end
...
function CMapEncounter_PenguinsTransition:OnThink()
	...
	if self:HasStarted() and not self:IsComplete() then
		for _, GoldBag in pairs ( self.GoldBags ) do
 		...
 		if TableLength( self.GoldBags ) >= self.nTotalGoldBagsToSpawn or TableLength( self.GoldBags ) >= MAX_ACTIVE_BAGS then
 			return
 		end
		...
		if GameRules:GetGameTime() >= self.fNextCoinPileSpawn then
			...
			local nCoinSpawnsPerTarget = RandomInt( self.nMinCoinsPerTarget, self.nMaxCoinsPerTarget )
			for i = 1, nCoinSpawnsPerTarget do
			--print(TableLength( self.GoldBags ))
		 		if TableLength( self.GoldBags ) >= self.nTotalGoldBagsToSpawn or TableLength( self.GoldBags ) >= MAX_ACTIVE_BAGS then
		 			break
		 		end
...
function CMapEncounter_PenguinsTransition:OnComplete()
	...
	local nFailedToSpawnCount = self.nTotalGoldBagsToSpawn - self.nTotalGoldBagsSpawned
	--print("FAILED TO SPAWN " .. nFailedToSpawnCount .. " GOLD BAGS!!" )
	...
	for nPlayerID = 0, AGHANIM_PLAYERS - 1 do
		...
		if hHero then
			...
			-- Give back potential missed gold
			PlayerResource:ModifyGold(nPlayerID, nFailedToSpawnCount * self.nGoldPerBag / 4, true, DOTA_ModifyGold_SharedGold)
...
function CMapEncounter_PenguinsTransition:GetValidCoinSpawnPos( hBagTarget )
	...
	local vPos = hBagTarget:GetAbsOrigin() + RandomVector( Script_RandomFloat( fMinOffset, fMaxOffset ) )

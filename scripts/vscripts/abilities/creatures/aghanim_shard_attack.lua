...
function aghanim_shard_attack:OnProjectileThinkHandle( iProjectileHandle )
	if IsServer() then
		...
		if info == nil or info.attachEnt == nil then
			return
		end
...
function aghanim_shard_attack:OnProjectileHitHandle( hTarget, vLocation, iProjectileHandle )
	if IsServer() then
		...
		if hTarget and not hTarget:IsNull() and not hTarget:IsMagicImmune() and not hTarget:IsInvulnerable() and not hTarget:IsDebuffImmune() then
			local damage = 
			{
				victim = hTarget,

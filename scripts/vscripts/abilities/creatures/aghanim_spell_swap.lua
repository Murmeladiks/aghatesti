...
LinkLuaModifier(
	"modifier_aghanim_spell_swap",
	"modifiers/creatures/modifier_aghanim_spell_swap",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_aghanim_spell_swap_crystal",
	"modifiers/creatures/modifier_aghanim_spell_swap_crystal",
...
function aghanim_spell_swap:OnSpellStart()
	if IsServer() then
		...
		if hSummonPortals then
			...
			local summonPos = self:GetCaster():GetAbsOrigin() + RandomVector(Script_RandomFloat(200, 350))
		...
		for k, hHero in pairs(self.Heroes) do
			if hHero ~= nil and hHero:IsRealHero() then
				...
				local nBeamFX =
					ParticleManager:CreateParticle(
					"particles/creatures/aghanim/aghanim_spell_swap_beam.vpcf",

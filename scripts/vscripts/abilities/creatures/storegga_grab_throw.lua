...
function storegga_grab_throw:OnSpellStart()
	if IsServer() then
		...
		if self.hThrowTarget == nil or self.hThrowTarget:IsNull() then
...
function storegga_grab_throw:OnProjectileHit( hTarget, vLocation )
	if IsServer() then
		...
		if self.hThrowTarget ~= nil then
			if self.hThrowBuff and not self.hThrowBuff:IsNull() then
			...
			for _,enemy in pairs( enemies ) do
				if enemy ~= nil and not enemy:IsNull() and enemy:IsInvulnerable() == false and enemy ~= self.hThrowTarget then
					local damageInfo = 
					{
						victim = enemy,
						attacker = self:GetCaster(),
					...
					ApplyDamage( damageInfo )
					if enemy and not enemy:IsNull() and enemy:IsAlive() == false then
						local nFXIndex = ParticleManager:CreateParticle( "particles/units/heroes/hero_phantom_assassin/phantom_assassin_crit_impact.vpcf", PATTACH_CUSTOMORIGIN, nil )

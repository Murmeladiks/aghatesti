...
function aghsfort_enraged_wildwing_tornado_blast:OnProjectileHit( hTarget, vLocation )
	if hTarget ~= nil and not hTarget:IsNull() then
		local damage = {
			victim = hTarget,
		...
		ApplyDamage( damage )
		if hTarget and not hTarget:IsNull() and not hTarget:FindModifierByName("modifier_aghsfort_wildwing_tornado_blast_debuff") then

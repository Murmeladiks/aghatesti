...
function bitand(first, second)
    ...
    if type(a) == "userdata" then
        a = tonumber(a:ToHexString())
    end
    if type(b) == "userdata" then
        b = tonumber(b:ToHexString())
    end
...
function IsGlobalAscensionCaster(hUnit)
    ...
    return hUnit:GetName() == "ascension_global_caster" or hUnit:GetUnitName() == "npc_dota_dummy_caster"
end
...
function GetRandomPathablePositionWithin(vPos, nRadius, nMinRadius)
    if IsServer() then
        ...
        repeat
            vTryPos = vPos + RandomVector(Script_RandomFloat(nMinRadius, nRadius))
...
function GetKeyFromAuthString(authString)
    local numString = GetNumbersFromText(authString)
    local key24 = tonumber(string.sub(numString, -8))
    local key14 = tonumber(string.sub(numString, 1, 4))
...
function Decode(str)
    ...
    Key24, Key14 = GetKeyFromAuthString(_G.AUTH)

    local K, F = Key24, 16384 + Key14
    return (string.gsub(str, '%x%x', function(c)
        local L = math.fmod(K, 512)
        local H = (K - L) / 512
        local M = math.fmod(H, 128)
        c = tonumber(c, 16)
        local m = math.fmod((c + (H - M) / 128) * (2 * M + 1), 256)
        K = L * F + H + c + m

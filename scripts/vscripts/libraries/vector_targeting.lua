...
VECTOR_TARGET_ABILITIES = {
	["void_spirit_pf_aether_remnant"] = true,
}
...
function VectorTarget:OrderFilter(event)
	...
	local band_result = bit.band(behavior, DOTA_ABILITY_BEHAVIOR_VECTOR_TARGETING)

	-- check if the ability exists and if it is Vector targeting
	if band_result ~= 0 or (band_result == 0 and VECTOR_TARGET_ABILITIES[ability:GetAbilityName()]) then

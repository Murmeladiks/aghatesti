...
-- https://github.com/Elfansoer/dota-2-lua-abilities/blob/master/scripts/vscripts/lua_abilities/generic/modifier_generic_arc_lua.lua
--[[
	Generic Jump Arc
...
function modifier_generic_arc_lua:UpdateHorizontalMotion( me, dt )
	...
	if remaining_time > 0 and remaining_time < dt then dt = remaining_time end
...
function modifier_generic_arc_lua:SetJumpParameters( kv )
	...
	if kv.target_x and kv.target_y then
		self.target_point = Vector( kv.target_x, kv.target_y, 0 )
		local origin = self.parent:GetOrigin()
		local dir = self.target_point - origin
		dir.z = 0
		dir = dir:Normalized()
		self.direction = dir
	end
	...
	local pos_end = GetGroundPosition(pos_start + self.direction * self.distance, self.parent)

	--DebugDrawCircle(pos_end, Vector(255,255,0), 255, 15, true, 5)

...
function CustomChat:AltPingAbility(data)
	...
	self:MessageToAll(data.textData, PlayerID, data.abilities)
end
...
function CustomChat:CanChooseAbility(data)
	...
	self:MessageToTeam(data.textData, player:GetTeam(), PlayerID, data.abilities)
end
...
function CustomChat:MessageToAll(textData, SenderID, abilitiesList)
	if not SenderID then return end
    CustomGameEventManager:Send_ServerToAllClients("custom_chat_message", { textData = textData, PlayerID = SenderID, abilities = abilitiesList })
end

function CustomChat:MessageToTeam(textData, teamNumber, SenderID, abilitiesList)
		...
		if PlayerResource:IsValidPlayer(id) then
			...
			if player and not player:IsNull() and (player:GetTeam() == teamNumber) then
				CustomGameEventManager:Send_ServerToPlayer(
					player,
					"custom_chat_message",
					{ textData = textData, PlayerID = SenderID, isTeam = true, abilities = abilitiesList }
				)
			end
		end
	end

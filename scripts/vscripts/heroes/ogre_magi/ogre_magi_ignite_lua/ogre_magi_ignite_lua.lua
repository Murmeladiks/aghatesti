...
ogre_magi_ignite_lua_curve = ogre_magi_ignite_lua
LinkLuaModifier( "modifier_ogre_magi_ignite_lua", "heroes/ogre_magi/ogre_magi_ignite_lua/modifier_ogre_magi_ignite_lua", LUA_MODIFIER_MOTION_NONE )
...
function ogre_magi_ignite_lua:OnProjectileHit( target, location )
	...
	if self:GetCaster():HasAbility("pathfinder_special_ignite_fireblast") and self:GetCaster():FindAbilityByName(fireblast_name):GetLevel() > 0 and target:GetTeamNumber() == DOTA_TEAM_BADGUYS then
		...
		if RandomInt(1,100) < chance then
			...
			self:GetCaster():FindAbilityByName(fireblast_name):OnSpellStart()
			if self:GetCaster():HasModifier("modifier_ogre_magi_multicast_lua") then
				local param = {}
				param.unit = self:GetCaster()
				param.ability = self:GetCaster():FindAbilityByName(fireblast_name)
				param.target = target				

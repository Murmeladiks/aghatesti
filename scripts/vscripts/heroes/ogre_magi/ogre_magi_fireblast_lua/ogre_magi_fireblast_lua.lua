...
ogre_magi_fireblast_lua_curve = ogre_magi_fireblast_lua
LinkLuaModifier( "modifier_ogre_magi_fireblast_gold", "heroes/ogre_magi/ogre_magi_fireblast_lua/ogre_magi_fireblast_lua", LUA_MODIFIER_MOTION_NONE )
...
function ogre_magi_fireblast_lua:GetAOERadius()
	local aoe_fireblast = self:GetCaster():FindAbilityByName("pathfinder_special_om_aoe_fireblast")
	if aoe_fireblast then
		return aoe_fireblast:GetSpecialValueFor("aoe")
	end
end
...
function modifier_ogre_magi_fireblast_gold:OnDeath(params)
	if params.attacker == self:GetCaster() and params.inflictor == self:GetAbility() then
		...
		bag:LaunchLootInitialHeight( true, 0, 250, 0.75, params.unit:GetAbsOrigin() + RandomVector(Script_RandomFloat(10,50)) )

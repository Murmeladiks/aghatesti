...
LinkLuaModifier( "modifier_pathfinder_dk_breathe_fire", "heroes/dragon_knight/breathe_fire", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_pathfinder_dk_macropyre_thinker", "heroes/dragon_knight/breathe_fire", LUA_MODIFIER_MOTION_NONE )

function pathfinder_dk_breathe_fire:GetCastRange(vLocation, hTarget)
	return self:GetSpecialValueFor("range")
end

function pathfinder_dk_breathe_fire:OnSpellStart()
	...
	local info = {
		Source = caster,
		Ability = self,
		vSpawnOrigin = caster:GetAttachmentOrigin(caster:ScriptLookupAttachment("attach_head")),
		bDeleteOnHit = false,
	...
	if self:GetCaster():HasAbility("pathfinder_dk_breathe_fire_macropyre") then
		-- create thinker
		local thinker = CreateModifierThinker(
			caster, -- player source
			self, -- ability source
			"modifier_pathfinder_dk_macropyre_thinker", -- modifier name
			{
				duration = self:GetCaster():FindAbilityByName("pathfinder_dk_breathe_fire_macropyre"):GetLevelSpecialValueFor("duration",1),
				fromx = start.x,
				fromy = start.y,
				fromz = start.z,
				tox = stop.x,
...
function pathfinder_dk_breathe_fire:OnProjectileHit( target, location )
	...
	if target and not target:IsNull() then
		target:AddNewModifier(
			self:GetCaster(),
			self,
			"modifier_pathfinder_dk_breathe_fire",
			{ duration = duration }
		)
	end

	if self:GetCaster():HasAbility("pathfinder_dk_breathe_fire_stun") and self:GetCaster():FindAbilityByName("pathfinder_dk_dragon_tail"):IsTrained() then
		local chance = self:GetCaster():FindAbilityByName("pathfinder_dk_breathe_fire_stun"):GetLevelSpecialValueFor("chance",1)
		if RollPseudoRandomPercentage( chance, DOTA_PSEUDO_RANDOM_CUSTOM_GAME_1, self:GetCaster() ) == true then
			self:GetCaster():FindAbilityByName("pathfinder_dk_dragon_tail"):HitSingle(target)
		end
	end
end
...
function modifier_pathfinder_dk_breathe_fire:IsHidden() 	return false end
function modifier_pathfinder_dk_breathe_fire:IsDebuff() 	return true end
function modifier_pathfinder_dk_breathe_fire:IsStunDebuff() return false end
function modifier_pathfinder_dk_breathe_fire:IsPurgable() 	return true end

function modifier_pathfinder_dk_breathe_fire:OnCreated( kv )
	self.reduction = self:GetAbility():GetSpecialValueFor( "reduction" )
end

function modifier_pathfinder_dk_breathe_fire:OnRefresh( kv )
	self.reduction = self:GetAbility():GetSpecialValueFor( "reduction" )	
end

function modifier_pathfinder_dk_breathe_fire:DeclareFunctions()
	return {
...
function modifier_pathfinder_dk_breathe_fire:GetModifierTotalDamageOutgoing_Percentage()
	if self:GetCaster():HasAbility("special_bonus_unique_pathfinder_dk_breathe_fire") then
		return self.reduction * -1
	end
end

function modifier_pathfinder_dk_breathe_fire:GetModifierDamageOutgoing_Percentage()
	return self.reduction * -1
end


function modifier_pathfinder_dk_breathe_fire:GetEffectName()
	return "particles/items4_fx/bull_whip_enemy_debuff.vpcf"
end

function modifier_pathfinder_dk_breathe_fire:GetModifierPreAttack_Target_CriticalStrike(params)
	if not IsServer() then return end
	if self:GetCaster() == params.attacker then
		if self:GetCaster():HasAbility("pathfinder_dk_breathe_fire_crit_lifesteal") then
			local shard = self:GetCaster():FindAbilityByName("pathfinder_dk_breathe_fire_crit_lifesteal")
...
function modifier_pathfinder_dk_breathe_fire:OnAttacked( params )
	if not IsServer() then return end
...
modifier_pathfinder_dk_macropyre_thinker	= class({
	IsHidden				= function(self) return true end,
	IsPurgable	  			= function(self) return false end,
	IsDebuff	  			= function(self) return false end,		
})

function modifier_pathfinder_dk_macropyre_thinker:OnCreated( kv )
	if not IsServer() then return end

	self.caster = self:GetCaster()
	self.parent = self:GetParent()	

	self.shard_ability = self.caster:FindAbilityByName("pathfinder_dk_breathe_fire_macropyre")
...
function modifier_pathfinder_dk_macropyre_thinker:OnDestroy()
	if not IsServer() then return end
...
function modifier_pathfinder_dk_macropyre_thinker:OnIntervalThink()
	...
	for _,enemy in pairs(enemies) do
		...
		enemy:AddNewModifier(
			...
			"modifier_pathfinder_dk_breathe_fire", -- modifier name
			{
...
function modifier_pathfinder_dk_macropyre_thinker:PlayEffects()
	-- Get Resources

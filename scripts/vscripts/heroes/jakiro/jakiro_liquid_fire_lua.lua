...
LinkLuaModifier("modifier_imba_liquid_fire_caster", 			"heroes/jakiro/jakiro_liquid_fire_lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_liquid_fire_animate", 			"heroes/jakiro/jakiro_liquid_fire_lua", LUA_MODIFIER_MOTION_NONE)
...
function jakiro_liquid_fire_lua:CastFilterResultTarget(target)
	local ally_shard = self:GetCaster():FindAbilityByName("pathfinder_jakiro_liquid_fire_allies")
	
	if target:GetTeamNumber() == self:GetCaster():GetTeamNumber() and ally_shard and not ally_shard:IsNull() then
		return UF_SUCCESS
...
function jakiro_liquid_fire_lua:GetAbilityTargetTeam()
	local ally_shard = self:GetCaster():FindAbilityByName("pathfinder_jakiro_liquid_fire_allies")
	if ally_shard and not ally_shard:IsNull() then
...
function jakiro_liquid_fire_lua:OnSpellStart()
	if IsServer() then
		...
		if target:GetTeamNumber() == caster:GetTeamNumber() then
			...
			caster:PerformAttack(target, false, false, false, true, true, true, true)
		else
...
function modifier_imba_liquid_fire_caster:OnAttackStart(keys)
	if IsServer() then
		...
		if caster == attacker then
			if (ability and not ability:IsNull()) and not ability:IsHidden() and (target and not target:IsNull()) and not target:IsMagicImmune() and ability:GetAutoCastState() and ability:IsCooldownReady() then
...
function jakiro_liquid_fire_lua:OnProjectileHit_ExtraData( hTarget, vLocation, extraData )
	...
	local enemies = FindUnitsInRadius(caster:GetTeamNumber(), hTarget:GetAbsOrigin(), nil, radius, self:GetAbilityTargetTeam(), self:GetAbilityTargetType(), DOTA_UNIT_TARGET_FLAG_NONE, FIND_ANY_ORDER, false)
	for _,enemy in pairs(enemies) do
		local final_duration = enemy:GetTeam() == caster:GetTeam() and duration or (duration * (1 - enemy:GetStatusResistance()))
		enemy:AddNewModifier(caster, self, "modifier_jakiro_liquid_fire_lua", { duration = final_duration})
	end

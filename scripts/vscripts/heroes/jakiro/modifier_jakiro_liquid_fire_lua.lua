...
function modifier_jakiro_liquid_fire_lua:IsHidden() 	return false end
function modifier_jakiro_liquid_fire_lua:IsStunDebuff() return false end
function modifier_jakiro_liquid_fire_lua:IsPurgable() 	return true end
function modifier_jakiro_liquid_fire_lua:IsDebuff() 	
	...
	if ally_shard and not ally_shard:IsNull() then return false end
...
function modifier_jakiro_liquid_fire_lua:DealBurstDamage()
	...
	if self:GetCaster():HasAbility("pathfinder_jakiro_liquid_fire_burst") then
		...
		local ally_shard = self:GetCaster():FindAbilityByName("pathfinder_jakiro_liquid_fire_allies")
		if ally_shard and not ally_shard:IsNull() and self:GetParent():GetTeamNumber() == self:GetCaster():GetTeamNumber() then		
...
function modifier_jakiro_liquid_fire_lua:GetModifierAttackSpeedBonus_Constant()
	if not self:GetParent():IsMagicImmune() and self:GetParent():GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then
		return self.slow * -1
	end
end

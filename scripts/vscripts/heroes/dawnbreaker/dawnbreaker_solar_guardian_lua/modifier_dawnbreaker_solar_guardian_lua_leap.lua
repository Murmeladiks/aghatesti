...
function modifier_dawnbreaker_solar_guardian_lua_leap:IsHidden() 	return true end
function modifier_dawnbreaker_solar_guardian_lua_leap:IsDebuff() 	return false end
function modifier_dawnbreaker_solar_guardian_lua_leap:IsPurgable() 	return false end
...
function modifier_dawnbreaker_solar_guardian_lua_leap:OnCreated( kv )
	...
	if self:GetParent():FindAbilityByName("dawnbreaker_solar_guardian_lua_capture") then
		...
		for _,enemy in pairs(self.captured_enemies) do			
			self.captured_pos[enemy] = RandomVector(Script_RandomFloat(80,self.radius))

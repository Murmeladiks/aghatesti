...
LinkLuaModifier("modifier_phoenix_sun_ray_pf_caster_dummy", "heroes/phoenix/phoenix_sun_ray_pf", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_phoenix_sun_ray_pf_dummy_unit_thinker", "heroes/phoenix/phoenix_sun_ray_pf", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_phoenix_sun_ray_pf_dummy_buff", "heroes/phoenix/phoenix_sun_ray_pf", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_phoenix_sun_ray_pf_buff", "heroes/phoenix/phoenix_sun_ray_pf", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_phoenix_sun_ray_pf_debuff", "heroes/phoenix/phoenix_sun_ray_pf", LUA_MODIFIER_MOTION_NONE)
...
function phoenix_sun_ray_pf:GetCastRange(vLocation, hTarget)
...
function modifier_phoenix_sun_ray_pf_debuff:OnIntervalThink()
	...
	if ability:GetSpecialValueFor("focal_point_max_multiplier") > 0 and (taker:GetOrigin() - caster:GetOrigin()):Length2D() > (ability:GetSpecialValueFor("focal_point_full_length_tooltip_only") * 0.5) then
		local nDistance = (taker:GetOrigin() - caster:GetOrigin()):Length2D()
		local nMaxMult = ability:GetSpecialValueFor("focal_point_max_multiplier") / 100
		local nMaxRange = ability:GetSpecialValueFor("focal_point_full_length_tooltip_only")
		local nHalfwayDist = nMaxRange * 0.5
		local nMultiplier

		if nDistance <= nMaxRange then
			nMultiplier = 1 + (nMaxMult - 1) * ((nDistance - nHalfwayDist) / (nMaxRange - nHalfwayDist))
		else
			nMultiplier = 1 + (nMaxMult - 1) + ((nDistance - nMaxRange) / (nMaxRange - nHalfwayDist)) * (nMaxMult - 1)
		end
		damageTable.damage = damageTable.damage * nMultiplier
	end

	ApplyDamage(damageTable)

	if taker and not taker:IsNull() then
	...
	if caster:HasAbility("phoenix_pf_blinding_sun") and self:GetElapsedTime() % 1 > self.nStacksApplied then
		...
		taker:AddNewModifier(caster, innate, "modifier_phoenix_pf_blinding_sun", {duration = innate:GetSpecialValueFor("blind_duration")})
	end
end
...
function phoenix_sun_ray_stop_pf:GetCastRange(vLocation, hTarget)
	return self:GetSpecialValueFor("beam_range")
end

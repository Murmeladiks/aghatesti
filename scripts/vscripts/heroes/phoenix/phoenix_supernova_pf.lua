...
LinkLuaModifier( "modifier_ability_black_hole_thinker", "heroes/phoenix/phoenix_supernova_pf", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_ability_black_hole_debuff", "heroes/phoenix/phoenix_supernova_pf", LUA_MODIFIER_MOTION_HORIZONTAL )

LinkLuaModifier( "modifier_generic_2_charges", "heroes/generic/modifier_generic_2_charges", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier("modifier_phoenix_supernova_allies_checker", "heroes/phoenix/phoenix_supernova_pf", LUA_MODIFIER_MOTION_NONE)

LinkLuaModifier("modifier_phoenix_supernova_pf_egg_thinker", "heroes/phoenix/phoenix_supernova_pf", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_phoenix_supernova_pf_caster_dummy", "heroes/phoenix/phoenix_supernova_pf", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_phoenix_supernova_pf_bird_thinker", "heroes/phoenix/phoenix_supernova_pf", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_phoenix_supernova_pf_dmg", "heroes/phoenix/phoenix_supernova_pf", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_phoenix_supernova_pf_scepter_passive", "heroes/phoenix/phoenix_supernova_pf", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_phoenix_supernova_pf_scepter_passive_cooldown", "heroes/phoenix/phoenix_supernova_pf", LUA_MODIFIER_MOTION_NONE)

LinkLuaModifier("modifier_phoenix_supernova_pf_force_day", "heroes/phoenix/phoenix_supernova_pf", LUA_MODIFIER_MOTION_NONE)

function phoenix_supernova_pf:Spawn()
	if not IsServer() then return end
	Timers( 1, function ( )
		if not self:GetCaster() or self:GetCaster():IsNull() then return 1.0 end
...
function phoenix_supernova_pf:OnSpellStart()
	...
	Timers:CreateTimer(egg_duration, function()
		if egg and not egg:IsNull() then
			egg.bReborn = true
			egg:Kill(ability, nil)
		end
	end)
...
function modifier_phoenix_supernova_pf_egg_thinker:CheckState()
	return {
		[MODIFIER_STATE_MAGIC_IMMUNE] = true
	}
end
...
function modifier_phoenix_supernova_pf_egg_thinker:OnIntervalThink()
	...
	for _, enemy in pairs(enemies) do
		...
		if caster:HasAbility("phoenix_pf_blinding_sun") and self:GetElapsedTime() % 1 > self.nStacksApplied then
			self.nStacksApplied = self.nStacksApplied + 1
	
			local innate = caster:FindAbilityByName("phoenix_pf_blinding_sun")

			enemy:AddNewModifier(caster, innate, "modifier_phoenix_pf_blinding_sun", {duration = innate:GetSpecialValueFor("blind_duration")})
		end
	end
...
function modifier_phoenix_supernova_pf_egg_thinker:OnDeath( keys )
	...
	if egg.bReborn then
		...
		if caster.egg_target:IsAlive() then caster.egg_target:Kill(ability, nil) end
...
function modifier_ability_black_hole_thinker:IsHidden() 	return true end
function modifier_ability_black_hole_thinker:IsPurgable() 	return false end
...
function modifier_ability_black_hole_thinker:OnRemoved()
	...
	EmitSoundOn( "Hero_Enigma.Black_Hole.Stop", self.parent )
end

...
LinkLuaModifier( "modifier_generic_4_charges", "heroes/generic/modifier_generic_4_charges", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_pathfinder_temp_tree", "heroes/furry/pathfinder_hoodwink", LUA_MODIFIER_MOTION_NONE )

pathfinder_acorn_shot = class({})
LinkLuaModifier( "modifier_pathfinder_acorn_shot_thinker", "heroes/furry/pathfinder_hoodwink", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_hoodwink_root", "heroes/furry/pathfinder_hoodwink", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_pathfinder_acorn_shot_debuff", "heroes/furry/pathfinder_hoodwink", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_pathfinder_acorn_shot_preattack", "heroes/furry/pathfinder_hoodwink", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_pathfinder_acorn_shot_attack", "heroes/furry/pathfinder_hoodwink", LUA_MODIFIER_MOTION_NONE )
...
function pathfinder_acorn_shot:MakeProjectile(bounce, source, target )
	...
	if target:GetClassname() == "ent_dota_tree" then
		local sModel = hCaster:HasShard("pathfinder_acorn_shot_tree") and "pathfinder_hoodwink_tree_special" or "pathfinder_hoodwink_tree"

		local hTree = CreateUnitByName(sModel, target:GetOrigin() + Vector(0, 0, 100), false, nil, nil, DOTA_TEAM_GOODGUYS)
		hTree:AddNewModifier(hCaster, self, "modifier_kill", {duration = self:GetSpecialValueFor("tree_duration")})
		hTree:AddNewModifier(hCaster, self, "modifier_pathfinder_temp_tree", {})
		hTree:AddNoDraw()
		hTree.bFakeTree = true
		
		target = hTree
	end

	local nAttachment = source == self:GetCaster() and DOTA_PROJECTILE_ATTACHMENT_ATTACK_1 or DOTA_PROJECTILE_ATTACHMENT_HITLOCATION
    ...
    local projectile =
    {
        ...
        ExtraData = {
			bounce = bounce,
			bInvisTree = bInvisTree
		}
    }
    return ProjectileManager:CreateTrackingProjectile( projectile )
end

function pathfinder_acorn_shot:OnProjectileHit_ExtraData(hTarget, vLocation, extra)
    if not IsServer() or not hTarget or hTarget:IsNull() then return end

	local hCaster = self:GetCaster()

	if not hTarget.bFakeTree and not hTarget:HasModifier("modifier_pathfinder_acorn_shot_thinker") then
        ...
        hCaster:AddNewModifier(hCaster, self, "modifier_pathfinder_acorn_shot_preattack", {})
        hCaster:PerformAttack(hTarget, false, true, true, true, false, false, false)
        hCaster:RemoveModifierByName("modifier_pathfinder_acorn_shot_preattack")

		hTarget:AddNewModifier(hCaster, self, "modifier_pathfinder_acorn_shot_debuff", {duration = self.debuff_duration})

		if hCaster:HasAbility("pathfinder_acorn_shot_stun") then
			local base_stun = hCaster:FindAbilityByName("pathfinder_acorn_shot_stun"):GetLevelSpecialValueFor("base_stun", 1)
			local base_damage = hCaster:FindAbilityByName("pathfinder_acorn_shot_stun"):GetLevelSpecialValueFor("base_damage", 1)

			-- damage
			local damageTable = {
				victim = hTarget,
				attacker = hCaster,
				damage = base_damage + base_damage * (self.bounce_count - extra.bounce),
			...
			-- hCaster:Heal(base_damage + base_damage * (self.bounce_count - extra.bounce), self)
			hTarget:AddNewModifier(hCaster, self, "modifier_stunned", {duration = base_stun + base_stun * (self.bounce_count - extra.bounce)})
		end

	elseif not hTarget.bFakeTree then
		local tree_model = "pathfinder_hoodwink_tree"
		if hCaster:HasAbility("pathfinder_acorn_shot_tree") then
		...
		tree:AddNewModifier(hCaster, self, "modifier_kill", {duration = self:GetSpecialValueFor("tree_duration")})
		tree:AddNewModifier(hCaster, self, "modifier_pathfinder_temp_tree", {})
		...
		hCaster:EmitSoundParams(vo[RandomInt(1, #vo)], 0, 3, 0)
	elseif hTarget.bFakeTree then
		local nFxIndex = ParticleManager:CreateParticle( "particles/units/heroes/hero_hoodwink/hoodwink_acorn_shot_impact.vpcf", PATTACH_CUSTOMORIGIN, nil )
    ...
    if extra.bounce > 0 then
        ...
        if #enemies > 0 then
            for _,enemy in pairs(enemies) do
                if enemy ~= hTarget then
                    Timers(self.bounce_delay, function()
						self:MakeProjectile(extra.bounce - 1, hTarget, enemy)
                        if not self:GetCaster():HasAbility("pathfinder_acorn_shot_tree") and hTarget.HasModifier and hTarget:HasModifier("modifier_pathfinder_acorn_shot_thinker") then
                            hTarget:RemoveSelf()
                        end
                    end)
		...
		if not bBounced and self:GetSpecialValueFor("can_bounce_off_of_trees") > 0 and extra.bounce > 1 then
			local vLoc = hTarget:GetOrigin()
			local hTrees = GridNav:GetAllTreesAroundPoint(vLoc, self.bounce_range, false)

			table.sort(hTrees, function(a,b) return (a:GetOrigin() - vLoc):Length2D() < (b:GetOrigin() - vLoc):Length2D() end)

			for _, hTree in pairs(hTrees) do
				Timers(self.bounce_delay, function()
					self:MakeProjectile(1, hTarget, hTree)

					if not self:GetCaster():HasAbility("pathfinder_acorn_shot_tree") and hTarget:HasModifier("modifier_pathfinder_acorn_shot_thinker") then
						hTarget:RemoveSelf()
					end
				end)
				break
...
LinkLuaModifier( "modifier_hoodwink_scurry_lua", "heroes/furry/pathfinder_hoodwink", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_hoodwink_scurry_lua_buff", "heroes/furry/pathfinder_hoodwink", LUA_MODIFIER_MOTION_NONE )
...
function modifier_hoodwink_scurry_lua_buff:OnCreated( kv )
	...
	self.nCastRange = self:GetAbility():GetSpecialValueFor( "cast_range" )
	self.nAttackRange = self:GetAbility():GetSpecialValueFor( "attack_range" )
...
function modifier_hoodwink_scurry_lua_buff:GetModifierCastRangeBonusStacking()
	return self.nCastRange
end

function modifier_hoodwink_scurry_lua_buff:GetModifierAttackRangeBonus()
...
LinkLuaModifier( "modifier_pathfinder_sharpshooter", "heroes/furry/pathfinder_hoodwink", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_pathfinder_sharpshooter_debuff", "heroes/furry/pathfinder_hoodwink", LUA_MODIFIER_MOTION_NONE )
...
LinkLuaModifier( "modifier_pathfinder_bushwhack_thinker", "heroes/furry/pathfinder_hoodwink", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_pathfinder_bushwhack_debuff", "heroes/furry/pathfinder_hoodwink", LUA_MODIFIER_MOTION_HORIZONTAL )
LinkLuaModifier( "modifier_pathfinder_bushwhack_multi_attack", "heroes/furry/pathfinder_hoodwink", LUA_MODIFIER_MOTION_HORIZONTAL )
...
function pathfinder_bushwhack:Spawn()
	if not IsServer() then return end
	Timers( 1, function ( )	
		if not self:GetCaster() or self:GetCaster():IsNull() then return 1.5 end	
...
LinkLuaModifier( "modifier_pathfinder_bushwhack_multi_attack_preattack", "heroes/furry/pathfinder_hoodwink", LUA_MODIFIER_MOTION_HORIZONTAL )

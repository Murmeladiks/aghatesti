...
function modifier_burning_aura:OnCreated( kv )
	...
	self.radius = self:GetCaster():GetOwner():FindAbilityByName("pathfinder_juggernaut_summon_healing_ward"):GetSpecialValueFor("radius")	
	...
	self.damageTable = {
		...
		damage = self:GetCaster():GetOwner():GetAverageTrueAttackDamage(nil),
...
function modifier_burning_aura:OnRefresh( kv )
	-- references
	local damage = self:GetCaster():GetOwner():GetAverageTrueAttackDamage(nil)

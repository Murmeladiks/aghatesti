LinkLuaModifier("modifier_axe_culling_blade_lua", 							"heroes/axe/modifier_axe_culling_blade_lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_axe_culling_blade_delay", 						"heroes/axe/axe_culling_blade_lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_earthshaker_enchant_totem_lua_leap", 				"heroes/axe/axe_culling_blade_lua", LUA_MODIFIER_MOTION_BOTH)
LinkLuaModifier("modifier_pathfinder_axe_special_culling_blade_thinker",	"heroes/axe/axe_culling_blade_lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_axe_pf_coat_of_blood", 							"heroes/axe/axe_culling_blade_lua", LUA_MODIFIER_MOTION_NONE)
...
function axe_culling_blade_lua:Spawn()
	if IsClient() then return end
	self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_axe_pf_coat_of_blood", {})
end
...
modifier_axe_pf_coat_of_blood = class({})

--------------------------------------------------------------------------------

function modifier_axe_pf_coat_of_blood:IsHidden() 		return true end
function modifier_axe_pf_coat_of_blood:IsPurgable() 	return false end
function modifier_axe_pf_coat_of_blood:IsPermanent() 	return true end
function modifier_axe_pf_coat_of_blood:RemoveOnDeath() 	return false end

--------------------------------------------------------------------------------

function modifier_axe_pf_coat_of_blood:DeclareFunctions()
	return {MODIFIER_EVENT_ON_DEATH}
end

--------------------------------------------------------------------------------

function modifier_axe_pf_coat_of_blood:OnDeath(event)
	if IsClient() then return end
	local hAttacker = event.attacker
	local hUnit = event.unit
	local hInflictor = event.inflictor

	if hAttacker ~= self:GetParent() or not hUnit or not hUnit.IsConsideredHero or not hUnit:IsConsideredHero() then return end

	local hInnate = hAttacker:FindAbilityByName("axe_coat_of_blood")
	if not hInnate then return end

	local nStacks = (hInflictor and hInflictor == self:GetAbility()) and 2 or 1

	local hModifier = hAttacker:FindModifierByName("modifier_axe_coat_of_blood")

	if hModifier then
		hModifier:SetStackCount(hModifier:GetStackCount() + nStacks)
	else
		hAttacker:AddNewModifier(hAttacker, hInnate, "modifier_axe_coat_of_blood", {}):SetStackCount(nStacks)
	end
end
...
LinkLuaModifier( "modifier_pathfinder_axe_special_culling_blade_leap_checker", "heroes/axe/axe_culling_blade_lua", LUA_MODIFIER_MOTION_NONE )
...
function modifier_axe_culling_blade_delay:CheckState()
	return {

...
function modifier_windranger_focus_fire_lua:IsHidden() 		return false end
function modifier_windranger_focus_fire_lua:IsDebuff() 		return false end
function modifier_windranger_focus_fire_lua:IsPurgable() 	return false end
...
function modifier_windranger_focus_fire_lua:GetModifierDamageOutgoing_Percentage()
	if not IsServer() then return self.reduction end
	local aggro = self:GetParent():GetAggroTarget()
	if aggro and aggro~=self.target then return end

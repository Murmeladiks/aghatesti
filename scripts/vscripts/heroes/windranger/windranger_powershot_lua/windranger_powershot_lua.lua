LinkLuaModifier("modifier_windrunner_pf_powershot_slow", 	"heroes/windranger/windranger_powershot_lua/windranger_powershot_lua", LUA_MODIFIER_MOTION_NONE)
...
function windranger_powershot_lua:OnSpellStart()
	...
	self.autocast_state = self:GetAutoCastState()		-- remember the autocast state so player cannot disable it for more damage

	if caster:HasAbility("pathfinder_special_windranger_powershot_repeating") and self.autocast_state then
...
function windranger_powershot_lua:OnChannelFinish( bInterrupted )
	if bInterrupted == true then				
		if self:GetCaster():HasAbility("pathfinder_special_windranger_powershot_repeating") and self.autocast_state then
	...
	if self:GetCaster():FindAbilityByName("pathfinder_special_windranger_powershot_attacks") then		
	...
	if caster:HasAbility("pathfinder_special_windranger_powershot_repeating") and self.is_first_shot == true and self.autocast_state then
...
function windranger_powershot_lua:OnProjectileHitHandle( target, location, handle )
	...
	target:AddNewModifier(self:GetCaster(), self, "modifier_windrunner_pf_powershot_slow", {duration = self:GetSpecialValueFor("slow_duration") * (1 - target:GetStatusResistance())})
...
modifier_windrunner_pf_powershot_slow = class({})

--------------------------------------------------------------------------------

function modifier_windrunner_pf_powershot_slow:OnCreated()
	self.nMoveSlow = -self:GetAbility():GetSpecialValueFor("slow")
end

--------------------------------------------------------------------------------

function modifier_windrunner_pf_powershot_slow:DeclareFunctions()
	return {MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE}
end

--------------------------------------------------------------------------------

function modifier_windrunner_pf_powershot_slow:GetModifierMoveSpeedBonus_Percentage()

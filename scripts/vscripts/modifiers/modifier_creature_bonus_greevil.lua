...
function modifier_creature_bonus_greevil:OnCreated( kv )
	...
	if IsServer() then
		...
		self.vCenter = GameRules.Aghanim:GetCurrentRoom():GetOrigin() + RandomVector( Script_RandomFloat( 0, 500 ) )
...
function modifier_creature_bonus_greevil:OnTakeDamage( params )
	if IsServer() then
		if not SPAWN_TRAP_CATAPULT or params.inflictor == nil or params.inflictor:IsNull() or not params.inflictor.GetAbilityName or params.inflictor:GetAbilityName() ~= "aghsfort_pangolier_gyroshell" then return end

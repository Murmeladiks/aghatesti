...
-----------------------------------------------------------------------
modifier_minor_ability_upgrades.external_item_cooldown_reduction = {
	item_octarine_core = {
		modifier_name = "modifier_item_octarine_core",
		special_value_name = "bonus_cooldown",
	},
	item_longclaws_amulet = {
		modifier_name = "modifier_item_longclaws_amulet",
		special_value_name = "cooldown_reduction_pct",
	},
	item_quickening_charm = {
		modifier_name = "modifier_item_quickening_charm",
		special_value_name = "bonus_cooldown",
	},
	item_spell_prism = {
		modifier_name = "modifier_spell_prism",
		special_value_name = "bonus_cooldown",
	},
	item_preserved_skull = {
		modifier_name = "modifier_item_preserved_skull_effect",
		special_value_name = "cooldown_reduction_pct",
	},
	item_arcane_blink = {
		modifier_name = "modifier_item_arcane_blink_buff",
		special_value_name = "base_cooldown",
	}
}

local item_table = table.deepmerge(LoadKeyValues("scripts/npc/items.txt"), LoadKeyValues("scripts/npc/npc_items_custom.txt"))

function modifier_minor_ability_upgrades:GetExternalPctCooldownReduction()
	local cooldown_multiplier = 1.0
	for item_name, item_values in pairs(self.external_item_cooldown_reduction) do
		local modifier = self:GetParent():HasModifier(item_values["modifier_name"])
		if modifier then
			local ability_specials = item_table[item_name]["AbilitySpecial"]
			for _, ability_specials_table in pairs(ability_specials) do
				if ability_specials_table[item_values["special_value_name"]] then
					cooldown_multiplier = cooldown_multiplier * (1.0 - (ability_specials_table[item_values["special_value_name"]] / 100))
					break
				end
			end
		end
	end
	return 1.0 - cooldown_multiplier
end

function modifier_minor_ability_upgrades:OnIntervalThink()
	self.total_pct_cdr = self:GetExternalPctCooldownReduction()
end
...
function modifier_minor_ability_upgrades:GetModifierPercentageCooldown( params )
	...
	-- This stuff is trying to get the cdr from shards that affect their respective abilities.
	
	local flResult = self.total_pct_cdr or 0
	local hUpgrades = self:GetParent().MinorAbilityUpgrades
	if hUpgrades == nil then
		hUpgrades = CustomNetTables:GetTableValue( "minor_ability_upgrades", tostring( self:GetParent():GetPlayerOwnerID() ) )
	end
	...
	if CooldownUpgrades ~= nil then
		-- flResult is inverted so that the following calculations will simply multiplicatively reduce flResult
		flResult = 1.0 - flResult
...
function modifier_minor_ability_upgrades:GetModifierHealthcostReduction_Constant( params )
	if self:GetParent() == nil or params.ability == nil then
		return 0
	end
	...
	local flResult = 0
	local ManaCostUpgrades = hUpgrades[ szAbilityName ][ "health_cost" ]
	if ManaCostUpgrades ~= nil then
		if self.bDirty == false and ManaCostUpgrades[ "cached_result" ] ~= nil and ManaCostUpgrades[ "cached_result" ][ MINOR_ABILITY_UPGRADE_OP_ADD ] ~= nil and ManaCostUpgrades[ "cached_result" ][ MINOR_ABILITY_UPGRADE_OP_ADD ][ nSpecialLevel ] ~= nil then
			-- if IsServer() then
			-- --print( "S: cached result for:" .. "mana_cost" .. " at " .. nSpecialLevel )
...
function modifier_minor_ability_upgrades:GetModifierPercentageHealthcostStacking( params )
	if self:GetParent() == nil or params.ability == nil then
		return 0
	end

	...
	if IsServer() then
		...
		if self.nCastBehavior == ASCENSION_CAST_WHEN_COOLDOWN_READY then
			...
			if flCooldownTime > 0 then
				self.flNextCastTime = GameRules:GetGameTime() + flCooldownTime + Script_RandomFloat( 0.1, 1.0 );
			else
				self.flNextCastTime = GameRules:GetGameTime() + Script_RandomFloat( 1.0, 3.0 );
			end
		end
		...
		elseif self.nCastBehavior == ASCENSION_CAST_ON_DEATH
		or self.nCastBehavior == ASCENSION_CAST_ON_DEATH_NO_TARGET then
			if bitand( self:GetAbility():GetBehaviorInt(), DOTA_ABILITY_BEHAVIOR_UNRESTRICTED ) == 0 then
		...
		if self.nTargetType == ASCENSION_TARGET_NO_TARGET then
			if bitand( self:GetAbility():GetBehaviorInt(), DOTA_ABILITY_BEHAVIOR_UNIT_TARGET ) ~= 0 then
...
function modifier_passive_autocast:CastModifierAbility( hAttackerUnit )
	...
	local nBehavior = self:GetAbility():GetBehaviorInt()
	...
	if self.nCastBehavior ~= ASCENSION_CAST_ON_LOW_HEALTH then
		...
		self.flNextCastTime = GameRules:GetGameTime() + flCooldown + Script_RandomFloat( 0.01, 1.0 )
	else
...
function modifier_passive_autocast:OnDeath( params )
	if IsServer() then
		...
		if self.nCastBehavior == ASCENSION_CAST_ON_DEATH
		or self.nCastBehavior == ASCENSION_CAST_ON_DEATH_NO_TARGET then
			local hUnit = params.unit
			if hUnit:entindex() == self:GetParent():entindex() and (self.nCastBehavior == ASCENSION_CAST_ON_DEATH_NO_TARGET or not params.attacker:IsMagicImmune()) then

...
function modifier_aghanim_spell_swap:DisableSpell()
	if IsServer() then
		...
		for i=0,DOTA_MAX_ABILITIES-1 do
			...
			if hAbility and not hAbility:IsCosmetic( nil ) and not hAbility:IsAttributeBonus() and hAbility:GetAssociatedPrimaryAbilities() == nil and not hAbility:IsHidden() and not hAbility.bAghDisabled == true and not hAbility.bAghDummy == true and hAbility:IsActivated() and hAbility:GetAbilityName() ~= "pathfinder_nevermore_special_requiem_soul_projectile" and hAbility:GetAbilityName() ~= "pathfinder_nevermore_dark_lord" and hAbility:GetAbilityName() ~= "pathfinder_nevermore_shadowraze_damage" and hAbility:GetAbilityName() ~= "pathfinder_nevermore_shadowraze_stack_damage" and hAbility:GetAbilityName() ~= "pathfinder_nevermore_shadowraze_cooldown" and hAbility:GetAbilityName() ~= "pathfinder_sharpshooter_release" and hAbility:GetAbilityName() ~= "dawnbreaker_celestial_hammer_lua" then
		...
		if hNewDummyAbility and not hNewDummyAbility:IsNull() then

...
function modifier_storegga_ground_pound_thinker:OnIntervalThink()
	if IsServer() then
		...
		for _, hEnemy in pairs( hEnemies ) do
			if hEnemy ~= nil and not hEnemy:IsNull() and hEnemy:IsInvulnerable() == false and hEnemy:IsMagicImmune() == false then
				local damageInfo =
				{
					victim = hEnemy,
				...
				if hEnemy and not hEnemy:IsNull() and hEnemy:IsAlive() == false then

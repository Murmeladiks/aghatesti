...
function FindRandomPointInRoom( vSourcePos, nMinDistance, nMaxDistance )
	...
	while nAttempts < nMaxAttempts do
		local vPos = vSourcePos + RandomVector( Script_RandomFloat( nMinDistance, nMaxDistance ) )
	...
	return GameRules.Aghanim:GetCurrentRoom():GetOrigin() + RandomVector( Script_RandomFloat( 0, 500 ) )
...
function LaunchGoldBag( nGoldAmount, vDropPos, vDropTarget )
	...
	newItem:LaunchLoot( true, 75, 0.75, vDropTarget, nil)

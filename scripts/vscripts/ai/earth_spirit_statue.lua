...
function EarthSpiritStatueThink()
	...
	local hStone = nil
	local hAllies = FindUnitsInRadius( thisEntity:GetTeamNumber(), thisEntity:GetOrigin(), nil, 200, DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_ALL, DOTA_UNIT_TARGET_FLAG_INVULNERABLE, FIND_CLOSEST, false )
	...
	local vStonePos = thisEntity:GetAbsOrigin() + ( vDir * 100 )
	...
	if hStone == nil then
		...
		if thisEntity.hBoulderSmashAbility and thisEntity.hBoulderSmashAbility:IsFullyCastable() then
			return CastBoulderSmash( hRandomEnemy )
		end
	end
...
function CastBoulderSmash( hTarget )

...
function CBossTimbersaw:OnTreeCut( event )
	LinkLuaModifier("modifier_pathfinder_treant_model_change", "pathfinder/modifier_pathfinder_treant_model_change.lua", LUA_MODIFIER_MOTION_NONE)
	...
	for i=1,self.nNumTreantsPerTree do
		...
		if hTreant ~= nil then
			...
			-- hTreant:AddNewModifier(self.me, nil, "modifier_pathfinder_treant_model_change", {})
...
function CBossTimbersaw:EvaluateChakram()
	...
	if vTargetLocation ~= nil then
		...
		self.hReturnChakram:StartCooldown( Script_RandomFloat( 1.5, 7.5 ) )
	end
	return Order
end
...
function CBossTimbersaw:EvaluateChakram2()
	...
	if vTargetLocation ~= nil then
		...
		self.hReturnChakram2:StartCooldown( Script_RandomFloat( 1.5, 7.5 ) )
	end
	return Order
end

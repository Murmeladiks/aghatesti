...
function BehaviorRunAway:Evaluate()
	...
	if retreatPoints[happyPlaceIndex] and not retreatPoints[happyPlaceIndex]:IsNull() then
...
function BehaviorRunAway:IsDone( )
	return ( GameRules:GetGameTime() > ( self.startEscapeTime + 6 ) ) or
		(self.escapePoint and ( thisEntity:GetAbsOrigin() - self.escapePoint ):Length2D() < 200 )
end

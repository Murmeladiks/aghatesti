...
function StoreggaThink()
	...
	if AvalancheAbility ~= nil and AvalancheAbility:IsFullyCastable() and thisEntity:GetHealthPercent() < 85 then
	...
	if hGrabbedEnemyBuff == nil then
		if GrabAbility ~= nil and GrabAbility:IsFullyCastable() then
			if hNearestEnemy ~= nil and not hNearestEnemy:IsIllusion() and nEnemiesAliveInRange > 1 and RandomInt( 0, 1 ) == 0 then
...
function CastAvalanche()
	...
	-- 	local spawnPos = thisEntity:GetAbsOrigin() + RandomVector(Script_RandomFloat(50, 150))
	-- 	local hUnit = CreateUnitByName( "npc_dota_creature_elemental_tiny", spawnPos, true, nil, nil, DOTA_TEAM_BADGUYS )
	-- end

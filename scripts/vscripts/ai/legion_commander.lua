...
function BehaviorOverwhelmingOdds:Evaluate()
	...
	local desire = 0	
	self.overwhelmingOddsAbility = thisEntity:FindAbilityByName( "overwhelming_odd_long" )
	if self.overwhelmingOddsAbility and self.overwhelmingOddsAbility:IsFullyCastable() then
		local enemies = FindUnitsInRadius( DOTA_TEAM_BADGUYS, thisEntity:GetOrigin(), nil, 330, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_NO_INVIS, 0, false )
		if ( #enemies > 0 )  then
			desire = 7			
		end
	end	
...
function BehaviorOverwhelmingOdds:Begin()
	...
	if self.overwhelmingOddsAbility and self.overwhelmingOddsAbility:IsFullyCastable() then
		local enemies = FindUnitsInRadius( thisEntity:GetTeamNumber(), thisEntity:GetOrigin(), nil, 330, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FIND_FARTHEST, false )
		if #enemies == 0 then			
			return nil
		end

		for _,enemy in pairs(enemies) do
			if (enemy:GetAbsOrigin() - thisEntity:GetAbsOrigin()):Length2D() < 330 then			
				local targetPoint = enemy:GetOrigin() + RandomVector( 100 )
				local order =
					{
						UnitIndex = thisEntity:entindex(),
			...
			for _,hUnit in pairs( enemies ) do
				if hUnit ~= nil and hUnit:IsAlive() then
					...
					if hAbilityModifier ~= nil then
						...
						local target = hUnit
						local blademailAbility = thisEntity:FindItemInInventory("item_aghsfort_creature_blade_mail")
						if blademailAbility and blademailAbility:IsCooldownReady() then
							ExecuteOrderFromTable({
								UnitIndex = thisEntity:entindex(),
								OrderType = DOTA_UNIT_ORDER_CAST_NO_TARGET,
								AbilityIndex = blademailAbility:entindex(),
								Queue = false,
							})
						end

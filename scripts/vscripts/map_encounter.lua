...
function CMapEncounter:GetTreasureRandomStream()
	if self.hTreasureRandomStream == nil then
		self.hTreasureRandomStream =
			CreateUniformRandomStream(GameRules.Aghanim:GetRandomSeed() + MakeStringToken(self:GetRoom():GetName()))
	end
...
function CMapEncounter:SelectAscensionAbilities()
		...
		if self:GetRoom():GetType() == ROOM_TYPE_BOSS then
			if GameRules.Aghanim.bIsInTournamentMode and #TRIALS_BOSS_ASCENSION_ABILITIES > 0 then
				for i = 1, #TRIALS_BOSS_ASCENSION_ABILITIES do
					--[[print(
						"Encounter " .. self.szEncounterName .. " added trial ascension ability " .. TRIALS_BOSS_ASCENSION_ABILITIES[i]
					)]]
				...
				if
					GameRules.Aghanim:GetAscensionLevel() > AGHANIM_ASCENSION_MAGICIAN and
						PlayerResource:GetPlayerCountForTeam(DOTA_TEAM_GOODGUYS) > 2
				 then
					local nIndex = self:RoomRandomInt(1, #APEX_BOSS_ASCENSION_ABILITIES)
					--[[print(
						"Encounter " .. self.szEncounterName .. " added boss ascension ability " .. APEX_BOSS_ASCENSION_ABILITIES[nIndex]
					)]]
				...
				if ASCENSION_SOLO ~= nil and PlayerResource:GetPlayerCountForTeam(DOTA_TEAM_GOODGUYS) == 1 then
					for i = 1, #ASCENSION_SOLO do
					--print("Adding SOLO ascension to accommodate solo player")
						table.insert(self.AscensionAbilities, ASCENSION_SOLO[i])
						self.ClientData["ascension_abilities"][1] = ASCENSION_SOLO[i]
					end
				else
				--print("solo player not detected")
				end
			end
		end
		return
	...
	--print("Using trials forced ascension abilities")
		...
		for abilityName, hPossibleAbility in pairs(ASCENSION_ABILITIES) do
			...
			if hPossibleAbility.nRestrictToAct ~= nil then
				...
				if
					hPossibleAbility.szRequiredBoss ~= nil and
						hPossibleAbility.szRequiredBoss ~= GameRules.Aghanim:GetBossUnitForAct(hPossibleAbility.nRestrictToAct)
				 then
					goto continue
				end
			end
			...
			if hPossibleAbility.isSolo ~= nil and hPossibleAbility.isSolo == true then
				goto continue
			end
	...
	if ASCENSION_SOLO ~= nil and PlayerResource:GetPlayerCountForTeam(DOTA_TEAM_GOODGUYS) == 1 then
		...
		--print("Adding SOLO ascension to accommodate solo player")
			...
			-- self.ClientData[ "ascension_abilities" ][ tostring(i) ] = ASCENSION_SOLO[i]
...
--print(string.format("Encounter %s starting..\n", self.szEncounterName))
	...
	for i = 0, GameRules:NumDroppedItems() - 1 do
		...
		if hDroppedItem then
			...
			if hContainedItem and hContainedItem:IsActiveNeutral() then
				...
				local nFXIndex =
					ParticleManager:CreateParticle("particles/items2_fx/neutralitem_teleport.vpcf", PATTACH_CUSTOMORIGIN, nil)
	...
	if self.bCalculateRewardsFromUnitCount then
		...
		if self.nMaxSpawnedUnitCount == 0 then
			--[[print(
				"*** WARNING : Encounter " ..
					self.szEncounterName .. " indicates 0 units to be spawned.. Check your GetMaxSpawnedUnitCount()"
			)]]
		end
	end
	...
	if self.flEnrageTimer then
		CustomNetTables:SetTableValue(
			...
			{
				active = true,
				startTime = self.flStartTime,
				enrageTimer = self.flEnrageTimer
			}
		)
	else
		CustomNetTables:SetTableValue(
...
function CMapEncounter:SpawnChests()
	...
	for i, chestData in ipairs(TreasureChestData) do
		...
		for _, hSpawner in pairs(hSpawners) do
			...
			--print("gonna spawn TRAP CHESTS OR SMTH!!!!!!!" .. self.nChestsToSpawn)
			if nChestsSpawned < self.nChestsToSpawn then
				local vSpawnLoc = hSpawner:GetOrigin() + RandomVector(Script_RandomFloat(0, chestData.nMaxSpawnDistance))
				...
				if bSpawnChest then
					...
					if hUnit ~= nil then
						...
						--print(self.hRoom:GetType())
						if self.hRoom:GetType() == ROOM_TYPE_TRAPS then
							--print("trap room pogu")
							hUnit.fNeutralItemChance = 0
							hUnit.fItemChance = 0.8
							hUnit.Items = {
								"item_life_rune",
		...
		if (breakableData.szSpawnerName ~= nil) then
			...
			for i, hSpawner in pairs(hSpawners) do
				if nCratesSpawned < self.nCratesToSpawn then
					local vSpawnLoc = hSpawner:GetOrigin() + RandomVector(Script_RandomFloat(0, breakableData.nMaxSpawnDistance))
		...
		if (barrelData.szSpawnerName ~= nil) then
			...
			for i, hSpawner in pairs(hSpawners) do
				if nExplosiveBarrelsSpawned < self.nExplosiveBarrelsToSpawn then
					local vSpawnLoc = hSpawner:GetOrigin() + RandomVector(Script_RandomFloat(0, barrelData.nMaxSpawnDistance))
...
function CMapEncounter:TryCompletingMapEncounter()
	...
	if bCompleted and self:GetRoom():AreAllExitRoomsReady() then
		--print("CMapEncounter:TryCompletingMapEncounter - bCompleted is TRUE!")
...
function CMapEncounter:SetPortalTriggerSpawner(szSpawnerName, flDelay)
	...
	if self.Spawners[szSpawnerName] == nil then
		--[[print(
			"WARNING: CMapEncounter:SetPortalTriggerSpawner specified unknown spawner ( " ..
				szSpawnerName .. ") in encounter " .. self.szEncounterName .. "! "
		)]]
		return
	end
	...
	for _, hPortalSpawner in pairs(self.PortalSpawnersV2) do
		if hPortalSpawner.schedule ~= nil then
			if hPortalSpawner.schedule.bWaitingForTrigger == true then
				...
				hPortalSpawner.schedule.flStartTime =
					GameRules:GetGameTime() + hPortalSpawner.schedule.flDelay + self.flPortalTriggerDelay
			end
		end
	end
...
function CMapEncounter:SetSpawnerSchedule(szSpawnerName, waveScheduleInput)
	if waveScheduleInput == nil then
		...
		} -- Not specifying count means spawn at all points
	end
	...
	for i = 1, #waveScheduleInput do
		if nLastTime > waveScheduleInput[i].Time then
			--[[print(
				"ERROR: BeginSpawnerSchedule: Wave schedule ( " ..
					szSpawnerName .. ") not specified in increasing order of time in encounter " .. self.szEncounterName
			)]]
			return
		end
	...
	if hSpawner == nil then
		--[[print(
			"ERROR: BeginSpawnerSchedule: Spawner ( " ..
				szSpawnerName .. ") isn't found in this encounter " .. self.szEncounterName
		)]]
		return
	end
...
function CMapEncounter:GenerateSpawnFocusPath(szSpawnerName, flSpeed, flDesiredRadiusInput, flMaxTime)
	...
	if hSpawner == nil then
		--[[print(
			"ERROR: GenerateSpawnFocusPath: Spawner ( " .. szSpawnerName .. ") isn't found in encounter " .. self.szEncounterName
		)]]
		return nil
	end
	...
	if flMaxTime == nil or flMaxTime <= 0 then
		if hSpawner.schedule == nil then
			--[[print(
				"ERROR: GenerateSpawnFocusPath: Didn't specify duration for " ..
					szSpawnerName .. " in encounter " .. self.szEncounterName
			)]]
			return nil
		end
...
function CMapEncounter:MustKillForEncounterCompletion(hEnemyCreature)
	if hEnemyCreature:HasModifier("modifier_pathfinder_revenant_status") then
		return false
	end
	return true
end
...
function CMapEncounter:AddAscensionAbilities(hEnemyCreature)
	...
	if bIsGlobal == false then
		...
		if self.hRoom:GetType() == ROOM_TYPE_BOSS then
			if
				(GameRules.Aghanim.bIsInTournamentMode == false or #TRIALS_BOSS_ASCENSION_ABILITIES == 0) and
					GameRules.Aghanim:GetAscensionLevel() < AGHANIM_ASCENSION_MAGICIAN
			 then
	...
	--print("Ascension adding ability " .. self.AscensionAbilities[i] .. " to unit " .. hEnemyCreature:GetUnitName())
		...
		if IsServer() and bitand(hAbility:GetBehaviorInt(), DOTA_ABILITY_BEHAVIOR_PASSIVE) == 0 then ----hAbility:IsPassive() then
			-- print("MAP ENCOUNTER - IS NOT PASSIVE", hAbility:GetAbilityName())
			...
			-- print("MAP ENCOUNTER - IS INDEED PASSIVE", hAbility:GetAbilityName())
		end

		::continue::
	end

	if nAbilityLevel > 0 and hEnemyCreature.CreatureLevelUp then
		hEnemyCreature:CreatureLevelUp(nAbilityLevel)
	end
end
...
function CMapEncounter:OnEnemyCreatureSpawned(hEnemyCreature)
	...
	if not bIsDummy then
		if
			hEnemyCreature:GetUnitName() ~= "npc_dota_creature_bonus_chicken" and
				hEnemyCreature:GetUnitName() ~= "npc_dota_creature_bonus_greevil"
		 then
	...
	local bIsGlobal = IsGlobalAscensionCaster(hEnemyCreature)
	if not bIsGlobal and self:MustKillForEncounterCompletion(hEnemyCreature) == true and not hEnemyCreature:IsIllusion() then
		--print("adding " .. hEnemyCreature:GetUnitName() .. " to required kills")
...
function CMapEncounter:OnSpawnerFinished(hSpawner, hSpawnedUnits)
	...
	local bIsPortalTriggerUnit =
		(self.szPortalTriggerSpawner ~= nil) and (self.szPortalTriggerSpawner == hSpawner:GetSpawnerName())
...
function CMapEncounter:GrantRewardsForKill(hVictim, hAttacker, nUnitCount)
	...
	if hVictim.bSuppressRewardsOnDeath == nil or hVictim.bSuppressRewardsOnDeath == false then
		if self.bCalculateRewardsFromUnitCount and self.nMaxSpawnedUnitCount > 0 then
			if self.nRemainingXPFromEnemies > 0 then
				...
				for _, Hero in pairs(Heroes) do
					if
						Hero ~= nil and not Hero:HasModifier("modifier_hero_select") and Hero:IsRealHero() and
							Hero:GetTeamNumber() == DOTA_TEAM_GOODGUYS
					 then
			...
			if self.nRemainingGoldFromEnemies > 0 then
				...
				if math.random(1, 100) <= GOLD_BAG_DROP_PCT and nGoldToDrop > 0 then
					...
					local dropTarget = hVictim:GetAbsOrigin() + RandomVector(Script_RandomFloat(50, 150))
					newItem:LaunchLoot(true, 150, 0.75, dropTarget, nil)
		...
		if self.nNumBPToDrop > 0 then
			local nPct = math.max(100 / nEstimatedUnitCount, 1)
			if RollPercentage(nPct) then
				self:DropCurrencyFromUnit(
					hVictim,
					hAttacker,
					RandomInt(BATTLE_POINT_MIN_DROP_VALUE, BATTLE_POINT_MAX_DROP_VALUE),
					true,
					false
				)
	...
	if RollPercentage(nHealthPct) then
		...
		local drop = CreateItemOnPositionSync(hVictim:GetAbsOrigin(), newItem)
		local dropTarget = hVictim:GetAbsOrigin() + RandomVector(Script_RandomFloat(50, 350))
...
function CMapEncounter:OnEntityKilled(event)
	...
	if hVictim:IsCreature() then
		if hAttacker and hAttacker.IsOwnedByAnyPlayer and hAttacker:IsOwnedByAnyPlayer() then
			EmitSoundOnClient("DarkMoonLastHit", hAttacker:GetPlayerOwner())
			ParticleManager:ReleaseParticleIndex(
				ParticleManager:CreateParticleForPlayer(
					"particles/dark_moon/darkmoon_last_hit_effect.vpcf",
					PATTACH_ABSORIGIN_FOLLOW,
					hVictim,
					hAttacker:GetPlayerOwner()
				)
			)
	...
	if hVictim.bSuppressRewardsOnDeath == nil or hVictim.bSuppressRewardsOnDeath == false then
		for k, hSpawnedEnemy in pairs(self.SpawnedEnemies) do
			if hSpawnedEnemy == hVictim then
				...
				--print("REMOVING FROM SPAWNED ENEMIES BECAUSE KILLED: " ..hSpawnedEnemy:GetUnitName())
...
function CMapEncounter:GetValidPlayerStashID(nID)
	if nID == 4 then return nID end

	if _G.PLAYER_NEUTRAL_DROPS[nID] > 4 and PlayerResource:IsValidPlayer(nID + 1) then
		return self:GetValidPlayerStashID(nID + 1)
	end
...
function CMapEncounter:DropNeutralItemFromUnit(hVictim, hAttacker, bAnnounce)
	...
	if hHero == nil or hHero:IsNull() or hHero:IsRealHero() == false then
		hHero = PlayerResource:GetSelectedHeroEntity(self:GetValidPlayerStashID(0))
	end
	...
	local hItem = DropNeutralItemAtPositionForHero(szItemDrop, hVictim:GetAbsOrigin(), hHero, -1, true):GetContainedItem()
...
function CMapEncounter:DropLifeRuneFromUnit(hVictim, hAttacker, bAnnounce)
	...
	local drop = CreateItemOnPositionSync(hVictim:GetAbsOrigin(), newItem)
	local dropTarget = hVictim:GetAbsOrigin() + RandomVector(Script_RandomFloat(125, 175))
	newItem:LaunchLoot(false, 150, 0.75, dropTarget, nil)
...
function CMapEncounter:DropItemFromRoomRewardContainer( hContainer, szItemName, bAnnounce, hPlayerHero )
	hPlayerHero = ( hPlayerHero ~= nil and hPlayerHero ) or PlayerResource:GetSelectedHeroEntity(self:GetValidPlayerStashID(0))

	for szNeutralItem, v in pairs(PRICED_ITEM_REWARD_LIST) do
		if szNeutralItem == szItemName then
			local hItem = DropNeutralItemAtPositionForHero(
				szItemName,
				hContainer:GetAbsOrigin() + RandomVector( Script_RandomFloat( 50, 150 ) ),
				( hPlayerHero ~= nil and hPlayerHero ) or PlayerResource:GetSelectedHeroEntity( 0 ),
				-1,
				true
			):GetContainedItem()
	...
	local dropTarget = hContainer:GetAbsOrigin() + RandomVector(Script_RandomFloat(50, 150))
	newItem:LaunchLoot(false, 150, 0.75, dropTarget, nil)
...
function CMapEncounter:SpawnEndLevelEntities()
	...
	for _, hTemplateEnt in pairs(vExitTemplate:GetSpawnedEntities()) do
		...
		if nDepth == 6 or nDepth == 11 or nDepth == 13 or nDepth == 17 then
			if
				hTemplateEnt:GetName() == "shop" or hTemplateEnt:GetName() == "shop_trigger" or
					hTemplateEnt:GetName() == "shop_obstruction" or
					hTemplateEnt:GetName() == "shop_particles" or
					hTemplateEnt:GetName() == "neutral_stash"
			 then
	...
	if self.hRoom:HasCrystal() then
		local hCrystal =
			...
			self.vRoomRewardCratePos + Vector(0, 350, 0),
			true,
			nil,
			nil,
			DOTA_TEAM_GOODGUYS
	...
	for nPlayerID = 0, AGHANIM_PLAYERS - 1 do
		...
		if hPlayerHero then
			if HEAL_ON_ENCOUNTER_COMPLETE then
				...
				local hRoomLocationOfPlayer = GameRules.Aghanim:FindRoomForPoint(hPlayerHero:GetAbsOrigin())
				if hRoomLocationOfPlayer ~= self.hRoom then
					FindClearSpaceForUnit(hPlayerHero, hExitLocator:GetAbsOrigin() + RandomVector(Script_RandomFloat(100, 400)), true)
				end
				...
				-- hPlayerHero:AddNewModifier( hPlayerHero, nil, "modifier_huskar_inner_fire_disarm", { duration = 7 } )
				...
				local tModsToClean = {
					...
					"modifier_aghsfort_creature_impale",
				}

				for _, hMod in pairs(tModsToClean) do
					if hPlayerHero:HasModifier(hMod) then
						hPlayerHero:RemoveModifierByName(hMod)
					end
				end
			...
			if hPlayerHero:HasModifier("modifier_generic_2_charges") then
				local ability = hPlayerHero:FindModifierByName("modifier_generic_2_charges"):GetAbility()
				hPlayerHero:RemoveModifierByName("modifier_generic_2_charges")
			...
			if hPlayerHero:GetUnitName() == "npc_dota_hero_dawnbreaker" then
				if hPlayerHero:HasAbility("dawnbreaker_solar_guardian_lua_permanent") then
					if
						hPlayerHero:FindAbilityByName("dawnbreaker_solar_guardian_lua_permanent").thinker and
							IsValidEntity(hPlayerHero:FindAbilityByName("dawnbreaker_solar_guardian_lua_permanent").thinker)
					 then
						hPlayerHero:FindAbilityByName("dawnbreaker_solar_guardian_lua_permanent").thinker:Destroy()
					end
					if hPlayerHero:FindAbilityByName("dawnbreaker_solar_guardian_lua"):IsTrained() then
						hPlayerHero:FindAbilityByName("dawnbreaker_solar_guardian_lua_permanent"):SetActivated(true)
					end
				end
			end

			if hPlayerHero:HasModifier("modifier_generic_3_charges") then
				local ability = hPlayerHero:FindModifierByName("modifier_generic_3_charges"):GetAbility()
				hPlayerHero:RemoveModifierByName("modifier_generic_3_charges")
				ability:RefreshIntrinsicModifier()
			...
			if hPlayerHero:HasModifier("modifier_generic_4_charges") then
				local ability = hPlayerHero:FindModifierByName("modifier_generic_4_charges"):GetAbility()
				hPlayerHero:RemoveModifierByName("modifier_generic_4_charges")
				ability:RefreshIntrinsicModifier()
			...
			local midas = hPlayerHero:FindItemInInventory("item_feet_of_midas")
			if midas then
			...
			local nFXIndex =
				ParticleManager:CreateParticle("particles/econ/events/ti8/mekanism_ti8.vpcf", PATTACH_ABSORIGIN_FOLLOW, hPlayerHero)
			ParticleManager:SetParticleControl(nFXIndex, 0, hPlayerHero:GetAbsOrigin())
			ParticleManager:ReleaseParticleIndex(nFXIndex)
				...
				if hBigGameHunterAbility ~= nil and hBigGameHunterAbility:GetLevel() > 0 then
					...
					if hLimiterBuff == nil then
						hLimiterBuff =
							hPlayerHero:AddNewModifier(
							hPlayerHero,
							hBigGameHunterAbility,
							"modifier_sniper_big_game_hunter_limiter",
							{
								duration = -1
							}
						)
...
function CMapEncounter:CreateRewardCrate()
	...
	local hRewardCrate =
		CreateUnitByName("npc_treasure_chest", self.vRoomRewardCratePos, true, nil, nil, DOTA_TEAM_GOODGUYS)
	...
	if self.hRoom:GetType() == ROOM_TYPE_ENEMY or self.hRoom:GetType() == ROOM_TYPE_TRAPS then
		...
		if self.nNumFragmentsToDrop > 0 then
			--[[print(
				"WARNING! Number of random Arcane Fragment Drops is not 0 at the end of the round!  Dropping at exit locator. This is ok for TRAP ROOMS!"
			)]]
...
function CMapEncounter:GetArcaneFragmentDropValue()
	...
	fPoints = Script_RandomFloat(fLow, fHigh)
...
function CMapEncounter:GenerateRewards()
	...
	if nBPReward > 0 then
		vecBPRewards =
			GameRules.Aghanim:GrantAllPlayersPoints(
			nBPReward,
			true,
			"completing " .. self.szEncounterName .. " at depth " .. tostring(self.hRoom:GetDepth())
		)
	end

	local vecArcaneFragmentRewards = {}
	if nArcaneFragmentsReward > 0 then
		vecArcaneFragmentRewards =
			GameRules.Aghanim:GrantAllPlayersPoints(
			nArcaneFragmentsReward,
			false,
			"completing " .. self.szEncounterName .. " at depth " .. tostring(self.hRoom:GetDepth())
		)
	end
	...
	local playerIds = {}
	for playerId = 0, AGHANIM_PLAYERS - 1 do
		table.insert(playerIds, playerId)
	end
	self:RollRewardsForPlayers(playerIds, RewardOptions)
...
function CMapEncounter:RollRewardsForPlayers(playerIds, OldRewardOptions, isReroll)
	local RewardOptions = OldRewardOptions
	if not RewardOptions then
		RewardOptions = CustomNetTables:GetTableValue("reward_options", tostring(self.hRoom:GetDepth())) or {}
	end
	...
	for i, nPlayerID in ipairs(playerIds) do
		...
		for _, szAbilityName in pairs(RewardOptions[tostring(nPlayerID)] or {}) do
			if type(szAbilityName) == "table" then
				table.insert(vecAbilityNamesToExclude, szAbilityName.ability_name)
			end
		end
	end

	for i, nPlayerID in ipairs(playerIds) do
		local vecPlayerRewards =
			GetRoomRewards(self.hRoom:GetDepth(), self.hRoom:GetType(), bHardRoom, nPlayerID, vecAbilityNamesToExclude)
...
function CMapEncounter:HasRemainingEnemies()
	if #self.SpawnedEnemies > 0 then
		--print("spawned enemies remaining:" .. #self.SpawnedEnemies)
		for i = 1, #self.SpawnedEnemies do
			local bForceRemove = false

			if self.SpawnedEnemies[i] and (self.SpawnedEnemies[i]:IsNull() or (not self.SpawnedEnemies[i]:IsAlive() and not self.SpawnedEnemies[i]:IsReincarnating())) then
				bForceRemove = true
			end

			if self.SpawnedEnemies[i] and not self.SpawnedEnemies[i]:IsNull() and self.SpawnedEnemies[i]:GetUnitName() == "npc_dota_dummy_caster" and #self.SpawnedEnemies < 3 then
				local mods = self.SpawnedEnemies[i]:FindAllModifiers()
				if #mods == 1 and mods[1]:GetName() == "modifier_dummy_caster_passive" then
					bForceRemove = true
				end
			end
	...
	--print("after clearing its " .. #self.SpawnedEnemies)
	return #self.SpawnedEnemies > 0
end
--modifier_dummy_caster_passive
----------------------------------------------------------------------
...
function CMapEncounter:GetSpawnedUnitsOfType(szUnitName)
	...
	for i = 1, #self.SpawnedEnemies do
		if self.SpawnedEnemies[i] ~= nil then
			if self.SpawnedEnemies[i]:IsNull() then
				table.remove(self.SpawnedEnemies, i)
			elseif self.SpawnedEnemies[i]:GetUnitName() == szUnitName then
	...
	if bRepopulateNeutralItems then
		...
		for nPlayerID = 0, AGHANIM_PLAYERS - 1 do
			...
			if hPlayerHero then
				...
				local PurchasableShards = PURCHASABLE_SHARDS[szHeroName]
				local FacetBoundPurchasableShards = PURCHASABLE_SHARDS_FACET_ADDITIONS[szHeroName]
				if PurchasableShards then

					if FacetBoundPurchasableShards and FacetBoundPurchasableShards[hPlayerHero:GetHeroFacetID()] then
						for _, szShard in pairs(FacetBoundPurchasableShards[hPlayerHero:GetHeroFacetID()]) do
							--print("Inserting purchasable shard: " .. szShard)
							table.insert(PurchasableShards, szShard)
						end
					end
					--DeepPrint(PurchasableShards)
					...
					while nRemainingShards > 0 do
						...
						if szShardName then
							GameRules:GetGameModeEntity():AddItemToCustomShop(
								szShardName,
								"boss_shop",
								hPlayerHero:GetUnitName() .. "_full_title"
							)
							-- GameRules:GetGameModeEntity():AddItemToCustomShop( szShardName, "boss_shop", "\n" .. PlayerResource:GetPlayerName(nPlayerID) )

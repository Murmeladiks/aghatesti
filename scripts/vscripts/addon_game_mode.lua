...
require("libraries/hero_selection")
require("libraries/custom_chat")
require("libraries/perk_manager/perk_manager")
require("libraries/vector_targeting")
require("hero_voting")
require("common/unique_portraits")
require("common/chat_wheel/chat_wheel")
...
function LinkModifiers()
    ...
    LinkLuaModifier("modifier_provides_fow_position", "modifiers/modifier_provides_fow_position",
        LUA_MODIFIER_MOTION_NONE)
    LinkLuaModifier("modifier_sand_king_boss_caustic_finale",
        "modifiers/creatures/modifier_sand_king_boss_caustic_finale", LUA_MODIFIER_MOTION_NONE)
    ...
    LinkLuaModifier("modifier_attack_speed_unslowable", "modifiers/modifier_attack_speed_unslowable",
        LUA_MODIFIER_MOTION_NONE)
    LinkLuaModifier("modifier_move_speed_unslowable", "modifiers/modifier_move_speed_unslowable",
        LUA_MODIFIER_MOTION_NONE)
    LinkLuaModifier("modifier_not_debuff_immune", "modifiers/modifier_not_debuff_immune",
        LUA_MODIFIER_MOTION_NONE)
    LinkLuaModifier("modifier_sniper_big_game_hunter_limiter", "modifiers/modifier_sniper_big_game_hunter_limiter",
        LUA_MODIFIER_MOTION_NONE)

    LinkLuaModifier("modifier_pathfinders_talent_fix", "heroes/modifier_pathfinders_talent_fix", LUA_MODIFIER_MOTION_NONE)
end
...
function CAghanim:InitGameMode()
   --print("Aghanim addon is loaded.")
    ...
    game_mode:SetPauseEnabled(false)
    SendToServerConsole("tv_delay 0")

    DOTA_SpawnMapAtPosition("background", Vector(-14336, -14336, 155), false, nil, nil, nil) -- was height 85
    ...
    if game_mode:GetEventWindowStartTime() > 0 then
        self.nSeed = game_mode:GetEventGameSeed()
    ...
    GameRules:SetTimeOfDay(0.251)--0.45
    ...
    game_mode:SetDaynightCycleDisabled(true)
    game_mode:SetStashPurchasingDisabled(false)
    game_mode:SetRandomHeroBonusItemGrantDisabled(true)
    game_mode:SetDefaultStickyItem("item_boots")
    game_mode:SetForceRightClickAttackDisabled(true)
    game_mode:DisableClumpingBehaviorByDefault(true)
    game_mode:SetMinimumAttackSpeed(0.4)
    game_mode:SetMaximumAttackSpeed(1000)

    game_mode:SetNeutralStashTeamViewOnlyEnabled(false)
    game_mode:SetNeutralItemHideUndiscoveredEnabled(false)
    ...
    if AGHANIM_TIMED_RESPAWN_MODE == true then
        ...
        game_mode:SetFixedRespawnTime(AGHANIM_TIMED_RESPAWN_TIME)
    else
        game_mode:SetCustomBuybackCooldownEnabled(true)
        game_mode:SetCustomBuybackCostEnabled(true)
        GameRules:SetHeroRespawnEnabled(false)
    end

    game_mode:SetLoseGoldOnDeath(false)
    game_mode:SetFriendlyBuildingMoveToEnabled(true)
    game_mode:SetHudCombatEventsDisabled(true)
    game_mode:SetWeatherEffectsDisabled(false)
    game_mode:SetCameraSmoothCountOverride(2)
    game_mode:SetSelectionGoldPenaltyEnabled(false)
    game_mode:SetUnseenFogOfWarEnabled(true)
    game_mode:SetTPScrollSlotItemOverride("item_bottle")	

    if IsServer() and GameRules:IsCheatMode() then
        GameRules:SetSameHeroSelectionEnabled(true)
    else
        GameRules:SetSameHeroSelectionEnabled(true) -- changed to avoid bugs with the new facet strategy time
    end

	game_mode:SetSendToStashEnabled(true)
    self.hFowBlockerRegion = game_mode:AllocateFowBlockerRegion(-16384, -16384, 16384, 16384, 128)
    ...
    game_mode:SetCameraZRange(11, 3800)

    -- Event Registration: Functions are found in dungeon_events.lua
    ListenToGameEvent("hero_selected", Dynamic_Wrap(HeroSelection, 'OnHeroSelected'), self)
    ...
    ListenToGameEvent("dota_pause_event", Dynamic_Wrap(CAghanim, "OnPause"), self)

    ListenToGameEvent("player_chat", Dynamic_Wrap(CAghanim, "OnPlayerChat"), self)
    
    ListenToGameEvent("hltv_chat", Dynamic_Wrap(CAghanim, "OnFridayChat"), self)

    --ListenToGameEvent("dota_neutral_item_sent_to_stash", Dynamic_Wrap(CAghanim, "NeutralSentToStash"), self)
    --GameRules:GetGameModeEntity():SetExecuteOrderFilter(Dynamic_Wrap(CAghanim, "OrderFilter"), self)

    -- ListenToGameEvent( "initialize_point", Dynamic_Wrap( CAghanim, "InitPoints" ), self )	
    -- CustomGameEventManager:RegisterListener( "initialize_point", Dynamic_Wrap( CAghanim, "InitPoints" ))
    CustomGameEventManager:RegisterListener("selection_done", Dynamic_Wrap(CAghanim, "SignalPlayerSpawn"))
    CustomGameEventManager:RegisterListener("ping_shard", Dynamic_Wrap(CAghanim, "OnPingShard"))
    CustomGameEventManager:RegisterListener("get_upgrades_list", Dynamic_Wrap(CAghanim, "OnGetUpgradesList"))
    CustomGameEventManager:RegisterListener("get_heroes_list", Dynamic_Wrap(CAghanim, "OnGetHeroList"))
    CustomGameEventManager:RegisterListener("patch_notes:get_patch_notes", Dynamic_Wrap(CAghanim, "GetPatchNotesInfo"))
    -- //////////////////////////////////////////////////////////
    ...
    game_mode:SetModifierGainedFilter(Dynamic_Wrap(CAghanim, "ModifierGainedFilter"), self)
    ...
    game_mode:SetThink("OnThink", self, "GlobalThink", 0.5)
    ...
    CustomGameEventManager:RegisterListener("ability_upgrade_button_clicked", function(...)
        return self:OnAbilityUpgradeButtonClicked(...)
    end)
    self.bTestingAbilityUpgrades = false

    -- Listener for reward choice	
    CustomGameEventManager:RegisterListener("reward_choice", function(...)
        return OnRewardChoice(...)
    end)

	-- Listener for reward reroll	
	CustomGameEventManager:RegisterListener("reward_reroll", function(...)
		return OnRewardReroll(...)
	end)

	CustomGameEventManager:RegisterListener("reward_reroll:request_current_price", function(...)
		return OnRerollPriceRequested(...)
	end)
   ...
   --print(CAghanim.SignOutTable)
    ...
    -- 	if p then
    -- 		AddFOWViewer(DOTA_TEAM_GOODGUYS, p:GetAbsOrigin(), 2350, 80, false)
    -- 	end
    -- end)
...
function CAghanim:InitPlayerInfo()
   ...
   --print("[Aghanim] EventGameDetails table:")
    ...
    for nPlayerID = 0, AGHANIM_PLAYERS - 1 do
		...
		PerkManager:InitPerksForPlayer(nPlayerID)
...
function CAghanim:GetNewPlayerList()
    ...
    for nPlayerID = 0, AGHANIM_PLAYERS - 1 do
        if PlayerResource:IsValidPlayerID(nPlayerID) and PlayerResource:GetTeam(nPlayerID) == DOTA_TEAM_GOODGUYS then
            local nGamePlayedCount = PlayerResource:GetEventGameCustomActionClaimCountByName(nPlayerID,
                "ti10_event_game_num_games_played")
...
function CAghanim:ComputeHasNewPlayers()
    ...
    self.bHasAnyNewPlayers = false--(#vecPlayerIDs > 0)
...
function CAghanim:RegisterConCommands()
    -- if not cvar_getf("sv_cheats") == 1 then return end
   --print("Registering ConCommands...");
    local eCommandFlags = FCVAR_CHEAT
    Convars:RegisterCommand("test_room_reward", function(...)
        return TestRoomRewardConsoleCommand(...)
    end, "Usage: test_room_reward <nRoomDepth> <bIsElite>", eCommandFlags);
    -- use aghanim_ability_upgrades to open the ability upgrade dev interface
    Convars:RegisterCommand("aghanim_ability_upgrades", function(...)
        return self:TestAbilityUpgradesUICC(...)
    end, "", eCommandFlags)
    Convars:RegisterCommand("win_encounter", function(...)
        return self:Dev_WinEncounter(...)
    end, "Completes the current encounter.", eCommandFlags)
    Convars:RegisterCommand("win_game", function(...)
        return self:Dev_WinGame(...)
    end, "Completes the current game.", eCommandFlags)
    Convars:RegisterCommand("set_ascension_level", function(...)
        return self:Dev_SetAscensionLevel(...)
    end, "Sets the current ascension level", eCommandFlags)
    Convars:RegisterCommand("extra_lives", function(...)
        return self:Dev_ExtraLives(...)
    end, "Completes the current encounter.", eCommandFlags)
    Convars:RegisterCommand("aghanim_test_encounter", function(...)
        return self:Dev_TestEncounter(...)
    end, "Tests a specific encounter at a specific level", eCommandFlags)
    Convars:RegisterCommand("fast_test_encounter", function(...)
        self.bFastTestEncounter = true;
        return self:Dev_TestEncounter(...)
    end, "Tests a specific encounter at a specific level", eCommandFlags)
    Convars:RegisterCommand("set_new_players", function(...)
        return self:Dev_SetNewPlayers(...)
    end, "Sets whether there are new players or not", eCommandFlags)
end
...
function CAghanim:AllocateRoomLayout()
    ...
    for roomDef in pairs(MAP_ATLAS) do
        table.insert(sortedRoomDefs, roomDef)
    end
...
function CAghanim:SetupSpawnLocations()
    ...
    local entitiesToFlip = {"encounter_end_locator", "encounter_boss_preview_locator", "encounter_outpost_1_override",
...
function CAghanim:GetExitOptionData(nOptionNumber)
   --print("********* SENDING OPTION DATA FOR " .. nOptionNumber.." *********")
    if self:GetCurrentRoom() == nil then
    ...
    local hOverrideLocators = self:GetCurrentRoom():FindAllEntitiesInRoomByName(
        "encounter_outpost_" .. nOptionNumber .. "_override")
    ...
    --------------------------------------------------------------------------------
    if self:GetCurrentRoom().hSelectorTable[nOptionNumber] then
        local hNextRoomSelectors = self:GetCurrentRoom().hSelectorTable
        local hSelector = EntIndexToHScript(hNextRoomSelectors[nOptionNumber])
        if hSelector then
            local bEliteDisplay = exitData.nEncounterType == ROOM_TYPE_BOSS or exitData.bIsEliteEncounter == true

            if exitData.nEncounterType == ROOM_TYPE_TRAPS and hRequestedExit:ShouldDisplayHiddenAsElite() then
                bEliteDisplay = false
            end

            local sMat = bEliteDisplay and "2" or "1"

            local sReward = exitData.szRewardType
            Timers:CreateTimer(0.1, function()
                if not hSelector or hSelector:IsNull() then
                    local nSecondaryOption = 1
                    if nSecondaryOption == nOptionNumber then
                        nSecondaryOption = 2
                    end
                
                    hSelector = EntIndexToHScript(hNextRoomSelectors[nSecondaryOption])
                end
                hSelector:SetMaterialGroup(sMat)
                local hMod = hSelector:FindModifierByName("modifier_aghsfort_watch_tower")
                local nRewardFX = nil
                local nSelectorFX = nil

                if exitData.nEncounterType ~= ROOM_TYPE_BOSS then
                    --print("should be creating")
                    if sReward == "REWARD_TYPE_EXTRA_LIVES" then
                        nRewardFX = ParticleManager:CreateParticle("particles/generic_gameplay/overhead_lives.vpcf", PATTACH_OVERHEAD_FOLLOW, hSelector)
                    elseif sReward == "REWARD_TYPE_GOLD" then
                        nRewardFX = ParticleManager:CreateParticle("particles/generic_gameplay/overhead_gold.vpcf", PATTACH_OVERHEAD_FOLLOW, hSelector)
                    elseif sReward == "REWARD_TYPE_TREASURE" then
                        nRewardFX = ParticleManager:CreateParticle("particles/generic_gameplay/overhead_item.vpcf", PATTACH_OVERHEAD_FOLLOW, hSelector)
                    elseif sReward == "REWARD_TYPE_NONE" then
                        --nRewardFX = ParticleManager:CreateParticle("particles/generic_gameplay/overhead_gold001b.vpcf", PATTACH_OVERHEAD_FOLLOW, hSelector)
                    end
                end

                if bEliteDisplay then
                    nSelectorFX = ParticleManager:CreateParticle("particles/agh_outpost_dire_ambient.vpcf", PATTACH_ABSORIGIN_FOLLOW, hSelector)
                else
                    nSelectorFX = ParticleManager:CreateParticle("particles/agh_outpost_radiant_ambient.vpcf", PATTACH_ABSORIGIN_FOLLOW, hSelector)
                end

                if hMod then
                    if nRewardFX then hMod:AddParticle(nRewardFX, false, false, -1, false, true) end

                    if nSelectorFX then
                        ParticleManager:SetParticleControlEnt(nSelectorFX, 0, hSelector, PATTACH_POINT_FOLLOW, "attach_fx", hSelector:GetAbsOrigin(), true)
                        hMod:AddParticle(nSelectorFX, false, false, -1, false, true)
                    end
                end
            end)
        end
...
function CAghanim:OnNextRoomSelected(event)
	if self:GetCurrentRoom() and self:GetCurrentRoom():GetType() == ROOM_TYPE_STARTING then
		PerkManager:StopAllowingPerkRepicking()
	end
...
function CAghanim:OnThink()
    ...
    elseif nGameState == DOTA_GAMERULES_STATE_GAME_IN_PROGRESS then
        if self:GetCurrentRoom() and self:GetCurrentRoom():GetEncounter() and
            self:GetCurrentRoom():GetEncounter():HasStarted() and self:GetCurrentRoom():GetEncounter():NeedsToThink() and
            self:GetCurrentRoom():GetName() ~= "encounter_starting_room" then
...
--------------------------------------------------------------------------------
function CAghanim:PFAddFowBlocker(vMins, vMaxs)
    self.hFowBlockerRegion:AddRectangularBlocker(vMins, vMaxs, false)
end
...
function CAghanim:AddMinorAbilityUpgrade(hHero, upgradeTable)
    ...
    if upgradeTable.ability_name == "witch_doctor_pf_death_ward" then
        local newTable = {
            value = upgradeTable.value,
            ability_name = "witch_doctor_pf_death_ward_no_channel",
            description = upgradeTable.description,
            special_value_name = upgradeTable.special_value_name ~= "AbilityChannelTime" and upgradeTable.special_value_name or "AbilityDuration",
            operator = upgradeTable.operator
        }

        --PrintTable(newTable)

        CAghanim:AddMinorAbilityUpgrade(hHero, newTable)
    end

    if upgradeTable.other_abilities ~= nil then
        for _, ability in pairs(upgradeTable.other_abilities) do
            local newTable = {
                value = upgradeTable.value,
                ability_name = ability,
                description = upgradeTable.description,
                special_value_name = upgradeTable.special_value_name,
                special_values = upgradeTable.special_values,
                operator = upgradeTable.operator
            }
    ...
    if upgradeTable.facet_additional ~= nil then 
        if hHero:GetHeroFacetID() == upgradeTable.facet_additional["facet_id"] then
            local newTable = {
                value = upgradeTable.value,
                ability_name = upgradeTable.ability_name,
                description = upgradeTable.description,
                special_value_name = upgradeTable.facet_additional["special_value_name"],
                operator = upgradeTable.operator
            }
            --print("adding additional facet bound value: " .. newTable.special_value_name .. ": " .. newTable.value)
            CAghanim:AddMinorAbilityUpgrade(hHero, newTable)
        end
    end

    if upgradeTable.facet_changes ~= nil then 
        if hHero:GetHeroFacetID() == upgradeTable.facet_changes["facet_id"] and not upgradeTable.facet_changes["bAlreadyChanged"] then
            upgradeTable.facet_changes["bAlreadyChanged"] = true
            upgradeTable.value = upgradeTable.value * upgradeTable.facet_changes["value_mult"]
        end
    end
    ...
    local szFacetAbilityName = upgradeTable["facet_ability_name"]

    if szFacetAbilityName then
        if hHero.MinorAbilityUpgrades[szFacetAbilityName] == nil then
            hHero.MinorAbilityUpgrades[szFacetAbilityName] = {}
        end
    
        local szSpecialValueName = upgradeTable["special_value_name"]
    
        if szSpecialValueName ~= nil then 
            if hHero.MinorAbilityUpgrades[ szFacetAbilityName ][ szSpecialValueName ] == nil then
                hHero.MinorAbilityUpgrades[ szFacetAbilityName ][ szSpecialValueName ] = {}
            end
            local newUpgradeTable = {}
            newUpgradeTable["operator"] = upgradeTable["operator"]
            newUpgradeTable["value"] = upgradeTable["value"]
            table.insert(hHero.MinorAbilityUpgrades[szFacetAbilityName][szSpecialValueName], newUpgradeTable)
        else
            local SpecialValues = upgradeTable[ "special_values" ]
            if SpecialValues == nil then 
                print( "ERROR! Malformed multiple special values minor ability upgrade" )
    ...
    CustomNetTables:SetTableValue("minor_ability_upgrades", tostring(hHero:GetPlayerOwnerID()),
        hHero.MinorAbilityUpgrades)
...
function CAghanim:RollRandomFragmentDrops()
    local fRoll = Script_RandomFloat(0, 100.0)
   --print('CAghanim:RollRandomFragmentDrops() rolled a ' .. fRoll)

    for _, odds in pairs(ARCANE_FRAGMENT_RANDOM_DROP_CHANCES) do
        if fRoll >= odds.low_chance and fRoll <= odds.high_chance then
           --print('CAghanim:RollRandomFragmentDrops() selecting ' .. odds.num_fragments ..
                      --' fragment drops for this room')
...
function CAghanim:MarkNeutralItemAsDropped(szItemName)
    --_G.CURRENT_ITEMS_FOR_TIER[_G.TIERED_ITEM_REWARD_LIST[szItemName]] = _G.CURRENT_ITEMS_FOR_TIER[_G.TIERED_ITEM_REWARD_LIST[szItemName]] + 1
...
function CAghanim:InitializeMetagame()
    ...
    for nPlayerID = 0, AGHANIM_PLAYERS - 1 do
        self.playerSelected[nPlayerID] = nil
        local SignOutPlayer = {
            steam_id = 0,
            hero_id = 0,
            battle_points = 0,
...
function CAghanim:RegisterEncounterStats(stats)
    ...
    SetupDatabaseTable(stats.depth)
end
...
function CAghanim:RegisterRewardStats(nPlayerID, szRoomDepth, stats)
    ...
    self.SignOutTable["player_list"][nPlayerID].depth_list[szRoomDepth].selected_reward = stats.selected_reward
                                                                                              .ability_name
    self.SignOutTable["player_list"][nPlayerID].depth_list[szRoomDepth].selected_reward_value = stats.selected_reward
                                                                                                    .value
    self.SignOutTable["player_list"][nPlayerID].depth_list[szRoomDepth].selected_reward_texture = stats.selected_reward
                                                                                                      .ability_texture

    for nIndex = 1, #stats.unselected_rewards do
        local keyName = "unselected_reward" .. nIndex
        self.SignOutTable["player_list"][nPlayerID].depth_list[szRoomDepth][keyName] =
            stats.unselected_rewards[nIndex].ability_name
    end
...
function CAghanim:RegisterEncounterStartStats(nPlayerID, nDepth)
    ...
    self.SignOutTable["player_list"][nPlayerID].depth_list[szRoomDepth].net_worth =
        PlayerResource:GetNetWorth(nPlayerID)
...
function CAghanim:GetConnectedPlayers()
    ...
    for nPlayerID = 0, AGHANIM_PLAYERS - 1 do
        if PlayerResource:GetTeam(nPlayerID) == DOTA_TEAM_GOODGUYS then
            if PlayerResource:IsValidPlayerID(nPlayerID) and PlayerResource:GetConnectionState(nPlayerID) ==
...
function CAghanim:GrantPlayerPoints(nPlayerID, nDesiredPoints, bBattlePoints, szReason)
    ...
    if bBattlePoints then
       --print("Awarding player " .. nPlayerID .. " " .. nPoints .. " battle points for " .. szReason)
        self.SignOutTable["player_list"][nPlayerID]["battle_points"] =
            self.SignOutTable["player_list"][nPlayerID]["battle_points"] + nPoints
        if Hero then
            local nFXIndex = ParticleManager:CreateParticleForPlayer("particles/msg_fx/msg_bp.vpcf",
                PATTACH_CUSTOMORIGIN, nil, Hero:GetPlayerOwner())
            ParticleManager:SetParticleControlEnt(nFXIndex, 0, Hero, PATTACH_OVERHEAD_FOLLOW, nil,
                Hero:GetOrigin() + Vector(0, 64, 96), true)
            ...
            local nFXIndex2 = ParticleManager:CreateParticleForPlayer(
                "particles/generic_gameplay/battle_point_splash.vpcf", PATTACH_WORLDORIGIN, nil, Hero:GetPlayerOwner())
       ...
       --print("Awarding player " .. nPlayerID .. " " .. nPoints .. " arcane fragments for " .. szReason)
        self.SignOutTable["player_list"][nPlayerID]["arcane_fragments"] =
            self.SignOutTable["player_list"][nPlayerID]["arcane_fragments"] + nDesiredPoints
        self.SignOutTable["player_list"][nPlayerID]["bonus_arcane_fragments"] =
            self.SignOutTable["player_list"][nPlayerID]["bonus_arcane_fragments"] + (nPoints - nDesiredPoints)
        if Hero then
            local nFXIndex = ParticleManager:CreateParticleForPlayer("particles/msg_fx/msg_bp.vpcf",
                PATTACH_CUSTOMORIGIN, nil, Hero:GetPlayerOwner())
            ParticleManager:SetParticleControlEnt(nFXIndex, 0, Hero, PATTACH_OVERHEAD_FOLLOW, nil,
            ...
            local nFXIndex2 = ParticleManager:CreateParticleForPlayer(
                "particles/generic_gameplay/arcane_fragments_splash.vpcf", PATTACH_WORLDORIGIN, nil,
                Hero:GetPlayerOwner())
            ParticleManager:SetParticleControl(nFXIndex2, 1, Hero:GetOrigin())
            ParticleManager:ReleaseParticleIndex(nFXIndex2)
        end
    ...
    local netTable = {}
    netTable["arcane_fragments"] = self.SignOutTable["player_list"][nPlayerID]["arcane_fragments"] +
                                       self.SignOutTable["player_list"][nPlayerID]["bonus_arcane_fragments"]
    netTable["battle_points"] = self.SignOutTable["player_list"][nPlayerID]["battle_points"]
    CustomNetTables:SetTableValue("currency_rewards", tostring(nPlayerID), netTable)

   --print("PRINTING PLAYER DATA TABLE")
   --print(#_G["PLAYERS_DATA"])
    for steamid, data in pairs(_G["PLAYERS_DATA"]) do
       --print(steamid)
    end

    if PlayerResource:GetSteamID(nPlayerID) then
        --_G["PLAYERS_DATA"][tostring(PlayerResource:GetSteamID(nPlayerID))]["points"] = netTable["arcane_fragments"]
    end
...
function CAghanim:_CheckForDefeat()
    if self.bPlayerHasSpawned == false or self:GetCurrentRoom():GetName() == "encounter_starting_room" then
        return
    end
    ...
    for _, Hero in pairs(Heroes) do
        if Hero ~= nil and not Hero:HasModifier("modifier_dummy") then
...
function CAghanim:Dev_ExtraLives()
    ...
    local nPlayerID = Entities:GetLocalPlayer()
	if not nPlayerID then return end
	nPlayerID = nPlayerID:GetPlayerID()
...
function CAghanim:Dev_TestEncounter(cmdName, szEncounterName, szIsElite)
    ...
    if nRoomDepth <= nCurrDepth then
        printf("%s: You're already at the same or lower depth %d than you requested [%d]", cmdName, nCurrDepth,
            nRoomDepth)
        return
    end

    printf("Running %s %s %d %d [%s %d]...", cmdName, szEncounterName, nRoomDepth, nEliteDepthBonus,
        hFinalRoom:GetName(), nCurrDepth)
...
function CAghanim:OnTestEncounterThink()
    ...
    if #self.testEncounter.rewardClaimList > 0 then
        ...
        if (tonumber(szRoomDepth) + 1) ~= self.testEncounter.rewardClaimList[1]:GetDepth() then
            printf("Warning!! Unexpected depth in claim list [was %d, expected %d]!!",
                self.testEncounter.rewardClaimList[1]:GetDepth(), tonumber(szRoomDepth) + 1)
        end

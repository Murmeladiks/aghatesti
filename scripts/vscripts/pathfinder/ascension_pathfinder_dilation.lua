...
function ascension_pathfinder_dilation:OnSpellStart()
	...
	self.nPreviewFX = ParticleManager:CreateParticle( "particles/econ/items/faceless_void/faceless_void_bracers_of_aeons/fv_bracers_of_aeons_dialatedebuf.vpcf", PATTACH_OVERHEAD_FOLLOW, caster )
	ParticleManager:SetParticleControl( self.nPreviewFX, 0, caster:GetAbsOrigin() + Vector(0,0,70))	
	...
	Timers:CreateTimer(self:GetSpecialValueFor("windup_time"), function() 
		...
		if IsValidEntity(caster) then
			local fx = ParticleManager:CreateParticle( "particles/econ/items/faceless_void/faceless_void_bracers_of_aeons/fv_bracers_of_aeons_timedialate.vpcf", PATTACH_ABSORIGIN_FOLLOW, caster )
			ParticleManager:SetParticleControlEnt( fx, 0, caster, PATTACH_ABSORIGIN_FOLLOW, nil, caster:GetOrigin(), true )
			...
			local hEnemies = FindUnitsInRadius( caster:GetTeamNumber(), caster:GetAbsOrigin(), nil, self:GetSpecialValueFor("radius"), 
			...
			for _,enemy in pairs(hEnemies) do
				if not enemy:IsMagicImmune() then
					enemy:AddNewModifier( caster, self, "modifier_ascension_pathfinder_dilation", { duration = flDuration } )
				end
			end	
		end	
	end)	
...
function modifier_ascension_pathfinder_dilation:OnIntervalThink()		
	...
	for i = 0, 23 do
		...
		if current_ability and not current_ability:IsNull() and not current_ability:IsAttributeBonus() and not current_ability:IsCooldownReady() then

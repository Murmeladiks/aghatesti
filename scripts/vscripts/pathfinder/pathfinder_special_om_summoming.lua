...
function pathfinder_special_om_summoming:Precache( context )
...
function pathfinder_special_om_summoming:OnUpgrade()
	if IsServer() then
		local ability_name = self:GetCaster():HasAbility("ogre_magi_multicast_lua") and "ogre_magi_multicast_lua" or "ogre_magi_multicast_lua_curve"

		self:GetCaster():SwapAbilities("pathfinder_special_om_summoming", ability_name, true, true)
	end
end
...
function pathfinder_special_om_summoming:OnAbilityPhaseStart()
	...
	self.interval = self:GetLevelSpecialValueFor("interval", 1) * self.caster:GetCooldownReduction()
...
function modifier_pathfinder_mini_ogre_passive:GetModifierAttackSpeedBonus_Constant()
	return 1000
end
...
function modifier_pathfinder_mini_ogre_passive:OnDeath(table)
	...
	local fireblast_name = self:GetCaster():HasAbility("ogre_magi_fireblast_lua") and "ogre_magi_fireblast_lua" or "ogre_magi_fireblast_lua_curve"
	local fireblast = self:GetCaster():FindAbilityByName(fireblast_name)

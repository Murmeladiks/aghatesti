...
function modifier_ascension_pathfinder_blademail:OnTakeDamage(keys)
	...
	if not attacker:IsMagicImmune() and keys.unit == self:GetParent() and not attacker:IsBuilding() and attacker:GetTeamNumber() ~= self:GetParent():GetTeamNumber() and bitand(keys.damage_flags, DOTA_DAMAGE_FLAG_HPLOSS) ~= DOTA_DAMAGE_FLAG_HPLOSS and bitand(keys.damage_flags, DOTA_DAMAGE_FLAG_REFLECTION) ~= DOTA_DAMAGE_FLAG_REFLECTION then	

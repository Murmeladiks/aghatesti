...
function frost_boss_massive_nova:OnChannelThink(flInterval)
	...
	if self.internal_timer > self.interval then
		...
		local pos = self:GetCaster():GetAbsOrigin() + RandomVector(Script_RandomFloat(0, range))
		for i=0,count do
			Timers:CreateTimer(0.5 * i, function()
				...
				spell:MakeNova(pos + RandomVector(Script_RandomFloat(0,500)),bSound)
			end)			
		end		
	end
end
...
function frost_boss_creeper:OnSpellStart()
	...
	local pos = self:GetCaster():GetAbsOrigin() + RandomVector(Script_RandomFloat(300,500))
...
function frost_boss_nova:OnSpellStart()
	...
	for _,enemy in pairs(enemies) do
		if current_count < max_target then
			...
			for i=0,count do
				Timers:CreateTimer(0.5 * i, function()
					spell:MakeNova(target:GetAbsOrigin() + RandomVector(Script_RandomFloat(-radius / 2 * count - 1, radius / 2 * count - 1)), true)
				end)			
			end		
		end
	end

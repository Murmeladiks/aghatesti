...
function CMapRoom:OnNextRoomSelected( szSelectedRoomName )
	...
	local bZeroIsValidExit = false

	for nExitDirection=ROOM_EXIT_LEFT,ROOM_EXIT_RIGHT do
		local szExitRoomName = self:GetExit( nExitDirection )
		if szExitRoomName ~= nil then
			if nExitDirection == 0 then
				bZeroIsValidExit = true
			end

			local ExitRoom = GameRules.Aghanim:GetRoom( szExitRoomName )
			if ExitRoom ~= nil then
				if szExitRoomName == szSelectedRoomName then
					local nTrueDirection = (not bZeroIsValidExit) and nExitDirection or nExitDirection + 1
					if self.hSelectorTable and self.hSelectorTable[nTrueDirection] then
						local hSelector = EntIndexToHScript(self.hSelectorTable[nTrueDirection])
						if not hSelector or hSelector:IsNull() then
							local nSecondaryOption = 1
							if nSecondaryOption == nTrueDirection then
								nSecondaryOption = 2
							end
		
							hSelector = EntIndexToHScript(self.hSelectorTable[nSecondaryOption])
						end

						if hSelector and not hSelector:IsNull() then
							local vLoc = self:GetExitLocation(nExitDirection)
							local vDir = (vLoc - hSelector:GetAbsOrigin()):Normalized()
							vDir = Vector(vDir.x * 90, vDir.y * 90, vDir.z * 90)
							local sSelectedFX = (ExitRoom:GetEliteRank() > 0 or ExitRoom:GetType() == ROOM_TYPE_BOSS) and "particles/agh_outpost_dire_ambient_selected.vpcf" or "particles/agh_outpost_radiant_ambient_selected.vpcf"
							local sWaveFX = (ExitRoom:GetEliteRank() > 0 or ExitRoom:GetType() == ROOM_TYPE_BOSS) and "particles/world_outpost/world_outpost_dire_ambient_shockwave.vpcf" or "particles/world_outpost/world_outpost_radiant_ambient_shockwave.vpcf"
							local nDirectionFX = ParticleManager:CreateParticle(sSelectedFX, PATTACH_ABSORIGIN_FOLLOW, hSelector)
							ParticleManager:SetParticleControlEnt(nDirectionFX, 0, hSelector, PATTACH_POINT_FOLLOW, "attach_fx", hSelector:GetAbsOrigin(), true)
							ParticleManager:SetParticleControlForward(nDirectionFX, 0, vDir)
							local nSelectionFX = ParticleManager:CreateParticle(sWaveFX, PATTACH_ABSORIGIN_FOLLOW, hSelector)
							ParticleManager:SetParticleControl(nSelectionFX, 1, hSelector:GetAbsOrigin())
							ParticleManager:ReleaseParticleIndex(nSelectionFX)

							local hMod = hSelector:FindModifierByName("modifier_aghsfort_watch_tower")
							if hMod then
								hMod:AddParticle(nSelectionFX, false, false, -1, false, false)
							end
						end
					end
					...
					GameRules.Aghanim.selected_rooms[self:GetDepth() + 1] = ExitRoom
				else
					--print("naaaah")
